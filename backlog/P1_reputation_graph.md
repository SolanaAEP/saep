...
- 2026-04-16: REST surface landed at `services/indexer/src/api.rs` — `GET /leaderboard?capability=<bit>&limit&cursor` (cursor = composite_score for keyset pagination), `GET /agents/:did/reputation` (per-capability rows), `GET /retro/eligibility/:operator`. Queries use raw `sql_query` against the materialized view (schema.rs only tracks legacy event tables). Hex-encoded pubkey IO, `tokio::spawn_blocking` around diesel, typed `ApiError` → JSON problem-responses. Merged into the existing axum router alongside /healthz + /metrics. Portal `useLeaderboard` / `useReputation` hooks remain pending.
