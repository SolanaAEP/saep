...
- 2026-04-16: SDK-UI hooks landed at `packages/sdk-ui/src/hooks/reputation.ts` — `useLeaderboard({ indexerUrl, capabilityBit, limit, cursor })`, `useAgentReputation({ indexerUrl, agentDidHex })`, `useRetroEligibility({ indexerUrl, operatorHex })` wrapping the indexer REST routes. camelCase/snake_case mapping at the boundary, 30s/60s staleTime, 404 → `null` on retro lookup. Exported from sdk-ui index. Portal leaderboard page `apps/portal/app/(app)/agents/leaderboard/page.tsx` + retro check page still pending.
