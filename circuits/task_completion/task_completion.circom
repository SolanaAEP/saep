...
template TaskCompletion(N_TASK, N_RESULT, K, LOG_K) {
    ...
    // 1. Commit to the private task witness so the circuit shape remains stable,
    // but do not currently constrain it against the public task_hash. The live
    // on-chain task hash is derived from task_id + payload hashing, and the
    // circuit rebinding for that schema is a follow-up.
