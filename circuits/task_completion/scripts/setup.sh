...
if [[ ! -f "$PTAU" ]]; then
  ...
  echo "dev-only-entropy-$(date +%s)" | npx snarkjs powersoftau contribute pot15_0000.ptau pot15_0001.ptau --name="saep-dev" -v
...
echo "dev-only-phase2-$(date +%s)" | npx snarkjs zkey contribute task_completion_0000.zkey task_completion.zkey --name="saep-dev" -v
