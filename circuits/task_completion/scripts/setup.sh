...
if [[ ! -f "$PTAU" ]]; then
  ...
  npx snarkjs powersoftau new bn128 "$PTAU_POWER" "pot${PTAU_POWER}_0000.ptau"
  echo "dev-only-entropy-$(date +%s)" | npx snarkjs powersoftau contribute "pot${PTAU_POWER}_0000.ptau" "pot${PTAU_POWER}_0001.ptau" --name="saep-dev"
  npx snarkjs powersoftau prepare phase2 "pot${PTAU_POWER}_0001.ptau" "$PTAU"
...
echo "dev-only-phase2-$(date +%s)" | npx snarkjs zkey contribute task_completion_0000.zkey task_completion.zkey --name="saep-dev"
