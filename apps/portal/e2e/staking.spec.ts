...
test.describe('staking surface', () => {
  test('renders the public mainnet staking rollout state', async ({ page }) => {
    ...
    await expect(page.getByTestId('staking-reward-rate')).toContainText(/0/i);
    ...
    await expect(page.getByText(/program on explorer/i)).toHaveCount(0);
    await expect(page.getByText(/rewards are not active yet/i)).toHaveCount(0);
