...
use crate::errors::NxsStakingError;
...
pub fn try_enter(guard: &mut ReentrancyGuard, caller: Pubkey, slot: u64) -> Result<()> {
    require!(!guard.active, NxsStakingError::GuardAlreadyActive);
    guard.active = true;
...
pub fn check_callee_preconditions(
    ...
    require!(
        ...
        NxsStakingError::CpiDepthExceeded
    );
    require!(
        allowed.programs.iter().any(|p| p == caller_program),
        NxsStakingError::UnauthorizedCaller
    );
    require!(caller_guard_active, NxsStakingError::CallerGuardNotActive);
    require!(!self_guard.active, NxsStakingError::ReentrancyDetected);
    Ok(())
}
...
pub fn assert_reset_timelock(guard: &ReentrancyGuard, now: i64) -> Result<()> {
    require!(
        ...
        NxsStakingError::AdminResetNotTimelocked
    );
