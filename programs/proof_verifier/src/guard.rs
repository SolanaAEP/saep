...
pub fn try_enter(guard: &mut ReentrancyGuard, caller: Pubkey, slot: u64) -> Result<()> {
    require!(!guard.active, ProofVerifierError::GuardAlreadyActive);
    guard.active = true;
...
pub fn check_callee_preconditions(
    ...
    require!(
        caller_guard_active,
        ProofVerifierError::CallerGuardNotActive
    );
    require!(!self_guard.active, ProofVerifierError::ReentrancyDetected);
    Ok(())
}
...
pub fn assert_reset_timelock(guard: &ReentrancyGuard, now: i64) -> Result<()> {
    require!(
        ...
        ProofVerifierError::AdminResetNotTimelocked
    );
...
mod tests {
    ...
    fn unauthorized_caller_rejected() {
        ...
        let intruder = Pubkey::new_from_array([2u8; 32]);
        assert!(check_callee_preconditions(&g, true, &intruder, &a, 2).is_err());
    }

    #[test]
    fn authorized_caller_accepted() {
        let g = new_guard();
    ...
    #[test]
    fn caller_guard_must_be_active() {
        let g = new_guard();
        let p = Pubkey::new_from_array([1u8; 32]);
        let a = allowed(&[p]);
