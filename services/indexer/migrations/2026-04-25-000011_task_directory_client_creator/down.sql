...
WITH created AS (
    ...
    ORDER BY data->'task_id', slot DESC, id DESC
),
latest AS (
    SELECT DISTINCT ON (data->'task_id')
        jsonb_u8_array_to_bytea(data->'task_id') AS task_id,
    ...
    ORDER BY
        ...
        slot DESC,
        CASE event_name
            WHEN 'TaskReleased'    THEN 80
            WHEN 'TaskVerified'    THEN 70
            WHEN 'ResultSubmitted' THEN 60
            WHEN 'DisputeRaised'   THEN 50
            WHEN 'TaskExpired'     THEN 45
            WHEN 'TaskCancelled'   THEN 40
            WHEN 'TaskFunded'      THEN 20
            WHEN 'TaskCreated'     THEN 10
            ELSE 0
        END DESC,
        id DESC
)
