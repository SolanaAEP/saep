...
Per-program folds follow a uniform spine: `A.x.1` program identity / `A.x.2` PDAs / `A.x.3` instructions / `A.x.4` events / `A.x.5` cross-program consumption / `A.x.6` invariants / `A.x.7` security checks (§1.2 mapping) / `A.x.8` known stubs / `A.x.9` test coverage / `A.x.10` finding-ledger filter. The three M1 programs with a Token-2022 *or* `alt_bn128` surface large enough to warrant its own treatment — task_market (Token-2022 + hook allowlist), treasury_standard (Token-2022 + Jupiter v6 + Pyth), proof_verifier (`alt_bn128` syscalls) — splice an extra sub-section in at A.x.6, pushing invariants and the remaining sections down one number; A.3 / A.4 / A.5 carry 11 sub-sections, A.1 / A.2 / A.6 / A.7 / A.8 carry 10. The structural shape captured at A.x.5 is the canonical CPI footprint per program: A.1 — read-target only (no CPI out); A.2 — Token-2022 only (4 sites); A.3 — sole live SAEP-internal CPI-out edge (to proof_verifier); A.4 — three CPI surfaces (Token-2022 + Jupiter v6 + Pyth read-only); A.5 — one CPI-out (to agent_registry) plus three direct bn254 syscall sites that are *not* CPI; A.6 — none-live + 4 forward-reserved cross-program pubkey pins (M2 scaffold, not yet deployed); A.7 — none-live + 1 minimal inline cross-program owner-check + 5 forward-reserved M2 CPI surfaces (M2 scaffold, not yet deployed); A.8 — Token-2022 only (2 sites) + zero forward-reserved cross-program pins (M2 scaffold, deployed-but-inert).
...
**Read targets** (inline reads, no CPI in either direction): `task_market` reads `AgentAccount` directly via Anchor `seeds::program = global.agent_registry` on `create_task` / `submit_result` / `release` / `expire` / `close_bidding` / `claim_bond`, verifying against `agent_did` / `operator` / `status` / `min_stake`; `task_market` also reads `PersonhoodAttestation` inline on `accept_task`. The inline-read pattern saves ~2k CU per call against an equivalent read-only CPI. The task_market-side counterpart of these edges is documented in A.3.5; treasury_standard's `init_treasury` AgentAccount inline read is in A.4.5.
...
4 findings touched agent_registry. Citations are IDs only; full entries live in §5.1.
...
- **Re-entrancy.** Critical surface — task_market is the largest CPI-out program in M1 by surface area (1 live CPI-out edge + 9 Token-2022 transfer sites + 3 inline AgentAccount reads + per-site fee_collector hook check). Self-guard `try_enter` / `guard_exit` on the 6 handlers that touch CPI (`submit_result`, `verify_task`, `release`, `expire`, `commit_bid`, `claim_bond`, plus implicit on `close_bidding` via its slash-loop transfers). **State-before-CPI** (Invariant 7) on every escrow-touching path: terminal status is written before any transfer; spec 07 §Re-entrancy explicitly mandates this ordering. **Cross-program parity callout:** task_market uses **only the const `MAX_CPI_STACK_HEIGHT = 3`** in `guard.rs`, no separate handler-local literal. Across the 4 M1 programs that carry guard surface (agent_registry / treasury_standard / proof_verifier / task_market) two distinct upper-bound conventions exist — agent_registry's `update_reputation` uses `<= 2`, proof_verifier carries both, treasury_standard + task_market use only the const `3` — surfaced in §3.2 + §6.2 as the binder's last surface to consolidate for external-reviewer ratification.
...
One-sentence summary: proof_verifier is the on-chain Groth16 bn254 validator that gates task settlement. It hosts a governance-rotatable verifying key under a 7-day timelock, runs pairing checks via direct `sol_alt_bn128_group_op` syscalls, validates public-input field-modulus bounds, and participates in the cross-program reentrancy guard DAG as the callee of `task_market::verify_task` and the caller of `agent_registry::update_reputation` (the proof-bound reputation rail). Smallest M1 program in surface area after capability_registry, and the only one with no Token-2022 / no oracle / no value held / no slashing surface; 20 public instructions (16 live + 2 chunked-VK upload + `finalize_batch` partial-implementation per §6.6 + 1 partial sibling), 4 `#[account]` PDAs + 2 embedded guard structs, 29 errors, 11 events, 3 `F-2026-NN` findings touched per §5.1 (F-2026-02 / 04 / 12 — all CLOSED).

#### A.5.1 Program identity

- **Program ID:** `DcJx1p6bcNuFm4i5WMgK4uGZitc1bf4Ubc5d4sctZKVe` (`programs/proof_verifier/src/lib.rs` `declare_id!`).
- **Spec:** [`specs/06-program-proof-verifier.md`](./specs/06-program-proof-verifier.md).
- **Upgrade authority on mainnet:** single deployer key today; rotation to a Squads 4-of-7 vault with the canonical 7-day timelock per [`specs/ops-squads-multisig.md`](./specs/ops-squads-multisig.md) is the next step (operational, not yet performed — see §7.3).
- **In-program `authority` (`VerifierConfig`):** begins as the deployer-supplied pubkey at `init_config`; two-step `transfer_authority` / `accept_authority`; migrates to the GovernanceProgram PDA once the M2 governance flow is initialised. Same pattern as capability_registry / agent_registry / treasury_standard / task_market.

#### A.5.2 PDAs

4 `#[account]` types in `state.rs` + 2 embedded reentrancy structs in `guard.rs`. All fixed-width `#[derive(InitSpace)]`; no `String` on-chain.

- **`VerifierConfig`** — governance singleton. Seeds `[b"verifier_config"]`. Carries `authority` + `pending_authority` (two-step transfer); `active_vk: Pubkey` (pointer to currently active `VerifierKey` PDA); `pending_vk: Option<Pubkey>` + `pending_activates_at: i64` (rotation staging — Invariant 2); `paused: bool`; `bump`.
- **`VerifierKey`** — per-VK content account. Seeds `[b"vk", vk_id]` where `vk_id: [u8; 32]` is governance-chosen (typically `sha256(VK bytes)`). Carries `vk_id` (immutable); pairing components `alpha_g1: [u8; 64]`, `beta_g2: [u8; 128]`, `gamma_g2: [u8; 128]`, `delta_g2: [u8; 128]`; `ic: Vec<[u8; 64]>` capped at `MAX_IC_LEN = 17` (= `MAX_PUBLIC_INPUTS = 16` + 1, Invariant 7); `num_public_inputs: u8`; `circuit_label: [u8; 32]` null-padded ASCII (versioning signal for public-input order); `is_production: bool` (mainnet-mode gate, Invariant 6); `registered_at`, `registered_by`; `bump`. Chunked upload path `init_vk` + `append_vk_ic × N` lands the IC in append-only segments because the post-F-2026-02 nine-public-input circuit grew IC past Solana's 1232-byte single-tx ceiling.
- **`GlobalMode`** — deployment flag singleton. Seeds `[b"mode"]`. Carries `is_mainnet: bool`, `bump`. Devnet builds set false; mainnet rejects `is_production = false` VK activations.
- **`BatchState`** — per-cranker batch accumulator. Seeds `[b"batch", cranker, batch_id]` (cranker-chosen `batch_id: [u8; 16]` independent of any task PDA). Carries `cranker` (creator + required signer for add/finalize/abort); `vk_key: Pubkey` (active VK at batch-open, pinned for consistency — `BatchVkMismatch` if active VK rotates mid-batch); `count`, `max_proofs` (1 ≤ max ≤ 10); pairing accumulators `acc_alpha`, `acc_vk_x`, `acc_c`; `random_state: [u8; 32]` (keccak chain seed for Fiat-Shamir scalars); `neg_a_scaled: Vec<[u8; 64]>` + `b_points: Vec<[u8; 128]>` (proof components staged for finalize); `bump`.
- **Embedded** (`guard.rs`): `ReentrancyGuard` (seeds `[b"guard"]`) + `AllowedCallers` (seeds `[b"allowed_callers"]`, `MAX_ALLOWED_CALLERS = 8`). Same shape as the agent_registry / treasury_standard / task_market guard pair; the cross-program guard DAG is documented in §3.

#### A.5.3 Instructions

20 live public instructions grouped 7-way by concern.

- **Init** (1): `init_config(authority, is_mainnet)` (deployer-signed; creates `VerifierConfig` + `GlobalMode` atomically).
- **VK lifecycle** (6): `register_vk(...)` (single-tx legacy path for VKs whose IC fits in 1232 bytes); `init_vk(vk_id, ...)` + `append_vk_ic(vk_id, ic_chunk)` (chunked upload — IC append-only, `IcLengthMismatch` enforced at activation); `propose_vk_activation()` (sets `pending_vk` + `pending_activates_at = now + 7d`; rejects if pending already set; on mainnet requires `vk.is_production == true`); `execute_vk_activation()` (permissionless crank; hard-gated `now ≥ pending_activates_at` per Invariant 3); `cancel_vk_activation()` (authority-signed; clears pending). All gated by `paused` except authority handoff.
- **Proof verification** (1, the hot path): `verify_proof(proof_a, proof_b, proof_c, public_inputs)` — any signer (in practice CPI from `task_market::verify_task`). **Read-only on-chain state** — no `&mut` accounts mutated, no `emit!` on success. Validates: passed VK matches `config.active_vk` (Invariant 1); public input count matches VK; every input is `< BN254_FIELD_MODULUS_BE` (`scalar_in_field`); proof points are well-formed curve elements (deserialisation in `pairing.rs`); 4-pair pairing equation evaluates to 1. Caller-guard DAG enforced before pairing (§3 + A.5.8); self-guard set on entry, cleared on exit.
- **Batch verification** (4): `open_batch(batch_id, max_proofs)` (cranker-signed; pins `vk_key = config.active_vk` at open-time; caps `max_proofs ≤ 10`); `add_batch_proof(...)` (same cranker; per-proof field-modulus checks + caches `(neg_a_scaled, B)` into accumulator; asserts `vk_key` still matches active); `finalize_batch()` — **partial M1 implementation** per §6.6 (emits `BatchVerified` + closes the PDA, but the actual N+3-pairing batch reduction is M2+; off-chain consumers MUST NOT consume the event as proof-of-validity); `abort_batch()` (closes PDA without `BatchVerified`).
- **Reputation rail** (1, live since `7c2143c`): `verify_and_update_reputation(proof_a, proof_b, proof_c, public_inputs, _agent_did, _capability_bit, sample, task_id)` — runs the F-2026-02 closure rail: (1) Groth16 verify against the active VK (9 public inputs); (2) reconstruct `agent_did` / `capability_bit` / `sample_hash` / `task_id` from `public_inputs[5..9]` — caller-supplied args are ignored, name-prefixed `_` to flag; (3) on-chain Poseidon over `sample` and assert equality with `sample_hash` from step 2; (4) CPI to `agent_registry::update_reputation`. Caller-guard shape `UncheckedAccount` + `load_caller_guard` (the F-2026-04 pattern). Mainnet-mode discipline check (`mode.is_mainnet ⇒ vk.is_production`) ensures the rail fails closed on mainnet until the multi-party ceremony lands a production VK.
- **Pause + authority** (3): `set_paused(paused)`; `transfer_authority(new_authority)`; `accept_authority()` (not pause-gated — handoff must not lock out governance).
- **Reentrancy guard management** (4): `init_guard(initial_callers)` (one-shot); `set_allowed_callers(programs)`; `propose_guard_reset()`; `admin_reset_guard()` — same 24h `ADMIN_RESET_TIMELOCK_SECS` pattern as the other M1 programs.

#### A.5.4 Events

11 events in `events.rs`, all decoded by `services/indexer` against the committed IDL: `VerifierInitialized`, `VkRegistered`, `VkActivationProposed`, `VkActivated`, `VkActivationCancelled`, `PausedSet`, `AuthorityTransferProposed`, `AuthorityTransferAccepted`, `BatchVerified`, `GuardEntered`, `ReentrancyRejected`. **No event on `verify_proof` happy path** — correct because the handler is read-only and the settlement event lives caller-side on `task_market`. All 11 covered by the indexer's borsh round-trip harness.

#### A.5.5 Cross-program consumption

**CPI out — one live surface.** `verify_and_update_reputation` (post-F-2026-02 close) CPIs into `agent_registry::update_reputation` after Groth16 verify + Poseidon binding. The 3 `sol_alt_bn128_group_op` calls in `pairing.rs` are direct Solana syscalls, not CPI (no Anchor `CpiContext`, no second program-id passed) — they remain syscalls; see A.5.6.

**CPI in — one live surface.** `task_market::verify_task → proof_verifier::verify_proof`. task_market passes 3 proof points + 5 public inputs in the locked order `task_hash, result_hash, deadline, submitted_at, criteria_root` per spec 06 §Public-input ordering and spec 07 §Interactions. task_market is the sole entry in `AllowedCallers` at M1. Caller's `ReentrancyGuard` must be active (validated via `load_caller_guard`); stack height ≤ 2 (F-2026-12 hard check at handler entry); self-guard inactive.

**Inline cross-program reads — none.** All reads are own-PDAs only (`VerifierConfig`, `VerifierKey`, `GlobalMode`, `ReentrancyGuard`, `AllowedCallers`). Distinct from treasury_standard which inline-reads `agent_registry::AgentAccount` at `init_treasury` (A.4.5).

#### A.5.6 bn254 syscall surface

Three direct `sol_alt_bn128_group_op` syscall sites in `pairing.rs` — `g1_add` (ALT_BN128_ADD, 128 in / 64 out, fixed-size stack buffers), `g1_scalar_mul` (ALT_BN128_MUL, 96 in / 64 out, fixed), `pairing_check` (ALT_BN128_PAIRING, slice-bounds enforced by AVM at the syscall boundary, 32-byte fixed output). One `negate_g1` site is bn254 field arithmetic only (modular subtraction — defined behaviour, not silent overflow). Inline `// nosemgrep:` annotation at the field-arithmetic site documents the in-house `saep.solana.wrapping-arithmetic` rule's intended exception. Per-site rationale also catalogued in [`.semgrep/README.md`](./.semgrep/README.md) so external reviewers reading the `.semgrep/` walkthrough land on the same justification text the source carries inline.

**No Token-2022 / no oracle / no slashing.** proof_verifier touches no mints, no token accounts, no Pyth feeds, no Switchboard feeds, no slashing destination. The integer-safety surface reduces to `scalar_in_field` (byte-by-byte big-endian comparison against `BN254_FIELD_MODULUS_BE` — no arithmetic) plus batch `count: u8` capped against `max_proofs ≤ 10` before increment plus `pending_activates_at` defensive `saturating_add` on `i64`.

#### A.5.7 Invariants

8 invariants: 7 from spec 06 §Invariants + 1 not-in-spec-but-in-code (#8), per the per-program convention of surfacing code-only invariants explicitly.

1. **`VerifierConfig.active_vk` always points to a `VerifierKey` whose account exists.** Enforced at `init_config` (initial pointer); `execute_vk_activation` swaps to `pending_vk` only when the timelock elapses and pending is set.
2. **`pending_vk.is_some() ⟺ pending_activates_at > 0`.** Set/cleared atomically by `propose_vk_activation` / `execute_vk_activation` / `cancel_vk_activation`.
3. **`execute_vk_activation` always enforces `now ≥ pending_activates_at`.** Hardcoded.
4. **`verify_proof` is pure — no account writes.** Enforced by handler signature (`Result<()>`, all account refs `&` not `&mut`); no `emit!` on the success path.
5. **Public-input count of the active VK never changes without a `circuit_label` change.** `register_vk` / chunked `init_vk` create a new `VerifierKey` with paired `(num_public_inputs, circuit_label)`; rotation swaps to an entirely new account. Changing the count requires a new label + new PDA.
6. **Mainnet-mode rejects `is_production == false` VK activation.** Enforced at `propose_vk_activation` against `GlobalMode.is_mainnet`. The same check is mirrored at the reputation-rail handler entry.
7. **`VerifierKey.ic.len() == num_public_inputs + 1` always.** Enforced at activation time with explicit `require!` (`IcLengthMismatch`); both fields immutable on the PDA after upload.
8. **Not in spec:** **Cross-program guard DAG: `verify_proof` validates caller via `load_caller_guard`** (owner = caller program, PDA derivation, discriminator, `active == true`) before entering the pairing path. Counterpart on the caller side is `task_market`'s self-guard (set on entry, cleared on exit). Same pattern as treasury_standard A.4.7 #8 and agent_registry — surfaced explicitly because the invariant lives across two programs.

#### A.5.8 Security checks (§1.2 mapping)

- **Account validation.** All 4 `#[account]` PDAs declare `seeds = [...]` + use stored `bump`. Anchor `Account<'info, T>` wrappers enforce discriminator + owner. The two guard structs in `guard.rs` use manual discriminator + owner checks for cross-program loads — exactly the F-2026-04 escape-hatch pattern. `verify_proof` asserts `passed_vk.key() == config.active_vk` to bind the supplied VK account to the active pointer (Invariant 1's runtime enforcement).
- **Authorization.** `authority` (stored in `VerifierConfig`, two-step transferable) gates: `register_vk`, `init_vk`, `append_vk_ic`, `propose_vk_activation`, `cancel_vk_activation`, `set_paused`, `transfer_authority`, `init_guard`, `set_allowed_callers`, `propose_guard_reset`, `admin_reset_guard`. `verify_proof` is permissionless to call as long as caller is in `AllowedCallers` and presents an active caller guard. Batch instructions gate on `cranker` signer (per-cranker batch isolation by seed). No ambient authority anywhere.
- **Re-entrancy.** Single self-guarded entry: `verify_proof`. Caller-guard DAG enforced via `load_caller_guard` (F-2026-04 pattern — `UncheckedAccount` + manual owner / PDA / discriminator / `active` checks; Anchor `Account<T>` would fail the cross-program owner check). `check_callee_preconditions` verifies stack height ≤ `MAX_CPI_STACK_HEIGHT = 3` (`guard.rs` const), AND `verify_proof` adds a separate hard `stack_height ≤ 2` check at handler entry as F-2026-12 hardening (defends against M2 chain expansion). **Cross-program parity callout** (cross-cited in §3.2 + §6.2 and A.4.8): `MAX_CPI_STACK_HEIGHT = 3` const here matches treasury_standard / task_market; `agent_registry::reputation.rs` uses literal `<= 2`. Two distinct upper-bound conventions across the four M1 programs that carry guard surface — surfaced for external-reviewer ratification. `BatchState` is per-cranker (seeds `[b"batch", cranker, batch_id]`), so cross-cranker batch reentrancy is impossible by construction; batch handlers don't enter the self-guard (no CPI surface).
- **Integer safety.** `scalar_in_field` is byte-by-byte big-endian comparison against `BN254_FIELD_MODULUS_BE` — no arithmetic, no overflow surface. Batch `count: u8` capped against `max_proofs ≤ 10` before increment; `max_proofs` itself capped at `MAX_BATCH_SIZE = 10`. `pending_activates_at` uses `saturating_add` on `i64` (defensive). No `checked_pow` / `checked_mul` surfaces — proof_verifier doesn't compute on user-supplied integers, only validates them; the §5.3 property-test real-finding lineage (treasury_standard A.4.8) does not have a counterpart here.
- **Upgrade.** Program upgrade authority rotates to Squads 4-of-7 with 7-day timelock per ops spec before mainnet. In-program VK rotation is *also* 7-day-timelocked (Invariant 3) — VK swaps follow the same governance cadence as program upgrades, by design.
- **Pause.** `VerifierConfig.paused` checked on every mutating ix (`register_vk`, `init_vk`, `append_vk_ic`, `propose_vk_activation`, `verify_proof`, batch ops) **except** authority handoff (`transfer_authority` / `accept_authority` — pause must not lock out governance). No carve-outs for exit-path liveness because there is no held value to refund.
- **Slashing.** N/A — proof_verifier does not slash anything.
- **Token-2022.** N/A — proof_verifier does not touch mints or token accounts.
- **Oracle.** N/A — proof_verifier performs no oracle reads. All inputs are caller-supplied; bounds checked (field modulus + IC length + public-input count).
- **PDA spoofing.** Stored-bump pattern on all 4 `#[account]` PDAs and both guard structs. `load_caller_guard` re-derives `find_program_address([b"guard"], caller_program)` and asserts equality — the only spot where dynamic PDA derivation happens, and it is the correct path.
- **Discriminator enforcement.** `fuzz.rs` covers all 4 account types with 7 plain `#[test]` (round-trip per type, all-truncated-to-discriminator-rejected, all-empty-buffers-rejected, discriminators-pairwise-distinct) plus 13 `proptest!` cases (arbitrary-discriminator rejection per type, correct-discriminator + random-tail no-panic per type, batch-state under random field values, extra-trailing-bytes Anchor-contract documentation). Default 256 invocations per proptest case → ~3,328 randomised runs against the deserialisation layer. Cross-cite of §4 matrix's `proof_verifier | 21 cases` row.

#### A.5.9 Known stubs

- **`finalize_batch`** — partial-implementation per §6.6. The handler emits `BatchVerified` and closes the `BatchState` PDA, but the actual N+3-pairing batch reduction is M2+. `open_batch` + `add_batch_proof` + `abort_batch` are fully functional (per-proof field-modulus checks + accumulator staging in `add_batch_proof`; PDA close in `abort_batch`). **The only stub family across the M1 in-scope set whose output is consumable by an off-chain caller in a misleading way** — explicitly disclosed in spec 06 §Batch verification + §6.6.
- **`verify_and_update_reputation`** — was DEFERRED (F-2026-02 fail-close path); rail is now live since `7c2143c` per A.5.3. No remaining stub surface.
- **Dead error variants.** `ProofVerifierError::NotImplemented` (planned escape hatch for future stubs; currently unused) and `ProofVerifierError::ReputationBindingNotReady` (was the F-2026-02 fail-close path before `7c2143c`; no longer thrown anywhere). Both harmless; cleanup deferred to keep the IDL stable through the audit window. Same retention policy as treasury_standard / task_market legacy variants (A.3.9 / A.4.9).
- No `TODO`, `FIXME`, `unimplemented!`, no feature-flagged dead branches.

#### A.5.10 Test coverage

**Rust unit + proptest (`cargo test -p proof_verifier`).** 50 unit/proptest cases across 6 files: `state.rs` (4 — `scalar_in_field` bounds at zero / modulus-1 / modulus / above modulus); `pairing.rs` (3 — `negate_g1` identity preservation, involutory, y-only change); `guard.rs` (10 — `try_enter` / `exit`, `check_callee_preconditions` (unauthorised / guard-inactive / reentrancy / stack-height), `load_caller_guard` (owner / PDA / discriminator validation), 24h timelock assertion); `instructions/batch_verify.rs` (4 — `derive_scalar` index-zero-is-one, upper-bytes-zeroed, deterministic, different-indices distinct); `instructions/reputation_cpi.rs` (9 — REP_AUTHORITY seed, capability-bit scalar encoding low+high, bool→scalar, `hash_sample` determinism + sensitivity); `fuzz.rs` (20 — 7 plain `#[test]` + 13 `proptest!` cases, see A.5.8 discriminator-enforcement). The §4 matrix counts the fuzz-side sub-total as 21 (8 plain plus 13 proptest, per current source); A.5.10's 7-plain-tests count tracks the `#[test]` attributes inside `fuzz.rs` proper.

**Anchor TS integration.** `tests/proof_verifier.ts` carries 4 active `it()` cases (program ID parity; `init_config` creates verifier config + mode; `register_vk` registers the dev-ceremony VK on-chain; `propose_vk_activation` sets pending VK with 7-day timelock) + 1 fixture-gated `.skip` (`verify_proof` happy-path with real proof — same blocker stack as A.4.10 / A.3.10's cross-program build fixture). The 7-day VK-rotate timelock has bankrun coverage in `tests/bankrun_timelocks.ts` (cycle 60 anchor `48716b7`); the 30-day slash bankrun adapter in agent_registry's surface is shared infrastructure, not duplicated here.

**Coverage gaps (known, not hidden).** No end-to-end `verify_proof` execution under Anchor localnet — three blockers: (1) `execute_vk_activation` requires bankrun for clock warp; (2) proof-gen service availability for fixture proofs; (3) `sol_alt_bn128_group_op` syscall is unavailable in bankrun — the §3 rider 4 in the audit-fix manifest's conditional-PASS section and the most material remaining test gap per public §6 / §7 (cross-cited from the canonical happy-path settlement chain caveat at line 153). No batch-verify integration test (`finalize_batch` partial-implementation; real batch pairing M2+). No `e2e_happy_path.ts` proof-verification leg under localnet (cross-program build-and-pray blocker; `tests/e2e_happy_path.ts` patches task state to `Verified` directly at the verify_task → verify_proof step). No mainnet-mode rejection test for `is_production = false` VK (Invariant 6).

#### A.5.11 Finding-ledger filter

3 findings touched proof_verifier. Citations are IDs only; full entries live in §5.1.

- **F-2026-02** (Critical, CLOSED in `7c2143c`; interim fail-close `41d18ff`). `verify_and_update_reputation` accepted unbound reputation args alongside a valid proof; closure rebound the circuit to 9 public inputs and rewrote the handler to reconstruct the bound tuple from `public_inputs[5..9]` plus on-chain Poseidon over `sample`. Caller-guard moved to the `UncheckedAccount` + `load_caller_guard` (F-2026-04) pattern. Mainnet ceremony gate (`mode.is_mainnet ⇒ vk.is_production`) is a separate ceremony-discipline check, not a F-2026-02 rider.
- **F-2026-04** (High, CLOSED in `41d18ff`; spawned F-2026-12). Cross-program `ReentrancyGuard` types failed Anchor's owner check (Anchor's `Account<T>` requires owner == declaring program; cross-program guards fail silently). Fix: `verify_proof` accepts `caller_guard: UncheckedAccount` and validates via `load_caller_guard` (owner = caller_program, PDA derivation, discriminator, `active == true`). Same fix applied symmetrically in agent_registry per A.2.7 (multi-program closure).
- **F-2026-12** (Low/Info, CLOSED in `5f008e8`). `caller_program` derivation via the instructions sysvar was structurally wrong — the sysvar lists top-level transaction instructions, not the CPI stack. Fix: hard `stack_height ≤ 2` rejection at `verify_proof` handler entry plus deriving `caller_program = current_ix.program_id` unconditionally. Latent (not exploitable today) because `load_caller_guard` double-checks owner + `guard.active`, so a wrong-caller path cannot produce a valid guard. Hardened anyway as part of the F-2026-04 sweep.

The conditional-PASS rider relevant to proof_verifier — that the `sol_alt_bn128_group_op` syscall is unavailable in bankrun and that no localnet integration coverage exercises the verify_task → verify_proof CPI edge with a real Groth16 proof — is the §3.4 / §6.1 caveat above (same anchor as A.2.10's cross-program guard DAG end-to-end gap; §6.4 covers the proof-binding-completeness re-check on the same edge).

### A.6 governance_program

One-sentence summary: governance_program is the M2 proposal-lifecycle program that other SAEP programs migrate their `authority` to once their M2 governance flow is initialised. It hosts a per-program registry, merkle-snapshot weighted voting, three tiered timelocks (7-day standard / 14-day critical / 21-day Meta) plus a 14-day execution window, and an 8-state proposal machine. **Posture: M2 scaffold — not yet initialised on mainnet** (see §2.1 row); 10 instructions, 5 `#[account]` PDAs, 9 events, 17 errors. **Zero `F-2026-NN` findings touched governance_program** at the time of this fold (the program was scaffolded after the M1 finding-ledger window — see A.6.10).

#### A.6.1 Program identity

- **Program ID:** `9uczLDZaN9EWqW76be75ji4vCsz3cydefbChqvBS6qw1` (`programs/governance_program/src/lib.rs` `declare_id!`).
- **Spec:** [`specs/program-governance.md`](./specs/program-governance.md).
- **Mainnet status:** **not yet deployed.** §2.1's M2 row lists the program as scaffolded; the M2 mainnet activation sequence (deploy + `init_config` + `register_program × N`) is the gating step that turns the on-chain governance flow live for the rest of the SAEP program set.
- **In-program `authority` (`GovernanceConfig`):** deployer-supplied at `init_config`; this program's PDA is the *destination* `authority` for the other SAEP programs' two-step `transfer_authority` migrations once the M2 flow is initialised. Distinct from the M1 programs' shape, where `authority` is the *source* of governance migrations.

#### A.6.2 PDAs

5 `#[account]` types in `state.rs`. All fixed-width `#[derive(InitSpace)]`; variable-length surfaces capped via `#[max_len(...)]`.

- **`GovernanceConfig`** — singleton. Seeds `[b"governance_config"]`. Carries `authority` + 4 cross-program pubkey pins (`nxs_staking`, `capability_registry`, `fee_collector`, `emergency_council`) + risk knobs (`min_proposer_stake`, `proposer_collateral`, three vote-window seconds (5d standard / 1d emergency / 7d Meta), `quorum_bps`, `pass_threshold_bps`, `meta_pass_threshold_bps`, three timelock-secs (7d / 14d / 21d), `min_lock_to_vote_secs = 30d`) + `dev_mode_timelock_override_secs` (per spec §4.3) + monotonic `next_proposal_id` + `next_emergency_id` + `paused` + `bump`.
- **`ProgramRegistry`** — singleton. Seeds `[b"program_registry"]`. Carries `entries: Vec<RegisteredProgram>` capped at `MAX_REGISTERED_PROGRAMS = 32`; each entry has `program_id`, 16-byte ASCII `label`, `is_critical: bool` (selects the 14-day timelock for ParameterChange / ProgramUpgrade / TreasurySpend / CapabilityTagUpdate proposals targeting it), 32-byte `param_authority_seed` (reserved metadata), `max_param_payload_bytes: u16` (per-program ix-data ceiling consumed by `propose`).
- **`ProposalAccount`** — per-proposal. Seeds `[b"proposal", proposal_id_le_bytes]`. Carries `proposer`, `category` (6-variant enum: ParameterChange / ProgramUpgrade / TreasurySpend / EmergencyPause / CapabilityTagUpdate / Meta), `target_program`, `ix_data: Vec<u8>` capped at `MAX_IX_DATA = 512`, `metadata_uri: Vec<u8>` capped at `MAX_METADATA_URI = 128`, `snapshot: ProposalSnapshot { total_eligible_weight: u128, snapshot_slot: u64, snapshot_root: [u8; 32] }` (off-chain-rooted voting-power tree), `status` (8-variant: Voting / Passed / Rejected / Queued / Executed / Failed / Cancelled / Expired), lifecycle timestamps (`created_at`, `vote_start`, `vote_end`, `tallied_at`, `executable_at`, `executed_at`), tallied vote weights (`for_weight` / `against_weight` / `abstain_weight`, all `u128`), `bump`.
- **`VoteRecord`** — per-(proposal, voter). Seeds `[b"vote", proposal_pubkey, voter_pubkey]`. Anchor-`init` semantics make double-vote structurally impossible. Carries `proposal_id`, `voter`, `choice` (For / Against / Abstain), `weight: u128` (matched against `merkle_proof` at vote time), `cast_at`, `bump`.
- **`ExecutionRecord`** — per-proposal-once. Seeds `[b"execution", proposal_pubkey]`. Anchor-`init` semantics make double-execution structurally impossible. Carries `proposal_id`, `executed_at`, `result: ExecutionResult` (4-variant: `Ok` / `CpiFailed{code: u32}` / `TargetMissing` / `PayloadInvalid`), `cpi_target`, `cpi_payload_hash: [u8; 32]` (sha256 over the recorded `ix_data` for off-chain reconciliation), `bump`.
- **No `ReentrancyGuard` / `AllowedCallers`** — distinct from agent_registry / treasury_standard / task_market / proof_verifier (the four M1 programs that carry guard surface). Justified by §A.6.5: the current scaffold's `execute_handler` performs no CPI dispatch (the recorded `ix_data` is stored but not invoked at M1).

#### A.6.3 Instructions

10 live public instructions grouped 4-way by concern. No stubs; all paths return `Result<()>` with structured errors.

- **Init** (1): `init_config(params)` (deployer-signed; creates `GovernanceConfig` + `ProgramRegistry` atomically; vote windows + timelocks + `min_lock_to_vote_secs` are hardcoded from spec §2.6 — caller-supplied params cover only the per-deployment knobs `authority` / cross-program pins / `min_proposer_stake` / `proposer_collateral` / quorum + threshold bps / `dev_mode_timelock_override_secs`).
- **Registry** (1): `register_program(label, program_id, is_critical, param_authority_seed, max_param_payload_bytes)` (`has_one = authority`; rejects duplicate `program_id`; rejects when `entries.len() >= MAX_REGISTERED_PROGRAMS`).
- **Proposal lifecycle** (5): `propose(category, target_program, ix_data, metadata_uri, snapshot)` (proposer-signed; pause-gated; rejects empty `metadata_uri`; rejects unregistered `target_program`; enforces `ix_data.len() <= entry.max_param_payload_bytes`; computes `vote_end = now + vote_window_for(category)`; bumps `next_proposal_id`); `vote(choice, weight, merkle_proof)` (any signer with a valid leaf; rejects when `merkle_proof.len() > MAX_MERKLE_PROOF_DEPTH = 24`; rejects when `now >= vote_end`; verifies `compute_vote_leaf(voter, weight)` against `snapshot.snapshot_root`; `init`s `VoteRecord` so double-vote rejects via Anchor); `finalize_vote()` (cranker-signed; rejects when `now < vote_end`; quorum check `total_voted >= eligible * quorum_bps / 10_000`; pass check `for_weight > (for + against) * threshold_for(category) / 10_000`; on pass sets `executable_at = now + timelock_for(category, is_critical)`; transitions Voting → Passed | Rejected); `queue_execution()` (cranker-signed; rejects if status ≠ Passed or `now < executable_at`; transitions Passed → Queued); `execute_proposal()` (executor-signed; rejects if status ≠ Queued or `now > executable_at + EXECUTION_WINDOW_SECS = 14d`; verifies `target_program` still in registry; init-once `ExecutionRecord`; transitions Queued → Executed | Failed; see A.6.8 for the M1 dispatch caveat).
- **Lifecycle exits** (3): `proposer_cancel()` (`has_one = proposer`; only valid before `vote_start` — the handler asserts `now <= proposal.created_at`, a one-tx cancellation window); `expire_proposal()` (cranker-signed; only after `executable_at + EXECUTION_WINDOW_SECS` elapses on a `Queued` proposal that was never executed); `set_paused(paused)` (authority OR `emergency_council` — explicit dual-path so the council can pause without round-tripping through the standard `authority`).

#### A.6.4 Events

9 events in `events.rs`, all decoded by `services/indexer` against the committed IDL: `ConfigInitialized`, `ProgramRegistered`, `ProposalCreated`, `VoteCast`, `ProposalFinalized`, `ProposalExecuted`, `ProposalCancelled`, `ProposalExpired`, `PausedSet`. Coverage by the indexer's borsh round-trip harness is queued with the M2 indexer schema bump (the M2 program set lands a single combined IDL update).

#### A.6.5 Cross-program consumption

**CPI out — none live in M1 scaffold.** `execute_handler` records `ExecutionRecord` and emits `ProposalExecuted{success:true}` without dispatching the proposal's recorded `ix_data` via CPI; the inline handler comment explicitly notes "for M2 we record success — real CPI wiring lands with program-specific handlers." This is the largest known stub in the program (cross-cited in A.6.8). The `ExecutionRecord.cpi_payload_hash` field captures sha256(`ix_data`) for off-chain reconciliation of *intent*, not on-chain execution.

**CPI in — none.** governance_program is the trust-anchor for the rest of the SAEP program set, but no other program calls into it. M2 migration moves each program's `authority` field to `governance_program`'s PDA; the dispatch model after that migration is governance-PDA-as-signer on the target program's existing setter ix surface, not a CPI back into governance_program.

**Inline cross-program reads — none.** The 4 cross-program pubkey pins in `GovernanceConfig` (`nxs_staking`, `capability_registry`, `fee_collector`, `emergency_council`) are stored at init-time but no current handler reads them against an external account; they are forward-reserved metadata for the M2 voting-snapshot binding (`nxs_staking`-rooted snapshot) and emergency-council pause path. Same posture as the four M1 cross-program ID pins on `agent_registry::RegistryGlobal` before each pin's M1 wiring landed.

**Distinguishing CPI shape vs the M1 folds:** A.1 (read-target only, no CPI surface), A.2 (Token-2022-only out + 1 CPI-in), A.3 (sole live SAEP-internal CPI-out), A.4 (3 CPI surfaces), A.5 (1 CPI-out + 1 CPI-in + 3 syscalls). A.6 carries **none-live + none-in + 4 forward-reserved pubkey pins** — the cleanest CPI shape in the appendix after A.1, by virtue of being scaffold-stage rather than by design.

#### A.6.6 Invariants

8 invariants: 4 from spec + 4 not-in-spec-but-in-code.

1. **Single proposal per `proposal_id`.** Anchor-`init` on the seed-derived PDA structurally rejects re-init.
2. **Single vote per `(proposal, voter)`.** Anchor-`init` on the seed-derived `VoteRecord` PDA structurally rejects double-vote without needing handler-side bookkeeping.
3. **Single execution per proposal.** Anchor-`init` on the seed-derived `ExecutionRecord` PDA.
4. **`config.next_proposal_id` strictly monotonic.** `checked_add` fall-through to `ArithmeticOverflow`.
5. **ProposalStatus is a one-way machine.** Legal transitions: Voting → {Passed | Rejected | Cancelled}; Passed → Queued; Queued → {Executed | Failed | Expired}. Each handler asserts the entry status; no handler accepts more than one prior status.
6. **`executable_at` is stored, not recomputed.** Set once at `finalize_vote` as `now + timelock_for(category, is_critical)`; subsequent `queue_execution` and `execute_proposal` read the stored value, so subsequent governance-config edits to the timelock secs do not retroactively shift the timelock window for in-flight proposals.
7. **Not in spec:** `EXECUTION_WINDOW_SECS = 14 days` is a hardcoded module-constant, not a `GovernanceConfig` field — surfaced explicitly so external review can ratify the choice. Set independently of the per-category timelock; both the `execute_proposal` deadline and `expire_proposal` precondition are computed from this single constant.
8. **Not in spec:** `proposer_cancel` is allowed only before `vote_start` (the handler asserts `now <= proposal.created_at`) — effectively a same-block-as-`propose` cancellation. Surfaced explicitly because the spec describes a more permissive "anytime before vote_start" window; the code is tighter.

#### A.6.7 Security checks (§1.2 mapping)

- **Account validation.** All 5 `#[account]` PDAs declare `seeds = [...]` + `bump = <stored>.bump`. `ProgramRegistry.entries` is `#[max_len(MAX_REGISTERED_PROGRAMS = 32)]`; `ProposalAccount.ix_data` and `metadata_uri` are bounded via `#[max_len]`. `propose` enforces a per-program `ix_data.len() <= entry.max_param_payload_bytes` ceiling on top of the global `MAX_IX_DATA = 512` cap (registry-driven cap).
- **Authorization.** `register_program` → `has_one = authority`; `set_paused` → authority *or* `emergency_council`; `proposer_cancel` → `has_one = proposer`; everything else (`propose` / `vote` / `finalize_vote` / `queue_execution` / `execute_proposal` / `expire_proposal`) is permissionless given the right state preconditions. No ambient authority anywhere; each governance-config edit is a separate ix not yet implemented in the scaffold (gated as A.6.8 follow-up).
- **Re-entrancy.** **N/A in M1 scaffold** — no CPI surface (A.6.5). When the M2 dispatch model lands, the guard surface will be added (the same `ReentrancyGuard` + `AllowedCallers` pattern as agent_registry / treasury_standard / task_market / proof_verifier; see §3.2 + §6.2 for the canonical pattern).
- **Integer safety.** `checked_add` on `next_proposal_id`, on every vote-weight accumulation (For / Against / Abstain `u128`), on `vote_end` and `executable_at` `i64` time deltas, on `EXECUTION_WINDOW_SECS` extension. `checked_mul` on the quorum-and-threshold ratio computations (`u128` intermediate over `total_eligible_weight * quorum_bps / 10_000`). Uniform `ArithmeticOverflow` error.
- **Upgrade.** Deployer → Squads vault with timelock per ops spec before mainnet. The `authority` field on `GovernanceConfig` is the program's *internal* authority for `register_program`; in-program governance migration (i.e. the program governing itself) is the Meta-category proposal path with a 21-day timelock (Invariant 6 cadence).
- **Pause.** `config.paused` checked on `propose` only. `vote` / `finalize_vote` / `queue_execution` / `execute_proposal` / `proposer_cancel` / `expire_proposal` deliberately bypass pause so in-flight proposals can complete their lifecycle — the same exit-path-liveness rationale as treasury_standard's pause carve-outs (A.4.8). `set_paused` itself is authority OR `emergency_council`.
- **Slashing.** **N/A in M1 scaffold** — `proposer_collateral` is initialised in `GovernanceConfig` but no handler reads or transfers it. Spec describes proposer-collateral slash on Rejected proposals; current scaffold accepts the parameter and does not enforce it. M2 expansion lands the deposit + slash + return path (see A.6.8).
- **Token-2022.** N/A — governance_program holds no token accounts.
- **Oracle.** N/A — governance_program performs no oracle reads. Snapshot-freshness validation against current slot is also currently not enforced (see A.6.8).
- **PDA spoofing.** Stored-bump pattern on all 5 PDAs. No `find_program_address` at runtime in the handlers.
- **Discriminator enforcement.** `fuzz.rs` covers all 5 `#[account]` types: 4 plain `#[test]` (pairwise-distinct discriminators across the 5 PDAs, empty-buffer rejection, vote-leaf determinism, single-leaf merkle proof correctness) plus 13 `proptest!` cases (arbitrary-bytes per primary PDA, wrong-discriminator rejection on `GovernanceConfig` / `ProposalAccount` / `VoteRecord`, vote-leaf determinism + voter-distinguishment + weight-distinguishment, bad-proof rejection with empty + sibling vectors). Default 256 invocations per proptest case → ~3,328 randomised runs against the deserialisation layer. The §4 matrix counts `governance_program | 18 cases`; this fold counts 17 (4 plain + 13 proptest, per current source) — counting-convention discrepancy explicitly disclosed.

#### A.6.8 Known stubs

- **`execute_handler` CPI dispatch — partial implementation.** The handler asserts the proposal is `Queued`, the timelock has elapsed, the execution window has not expired, and the target is still registered. On registered-target it sets `status = Executed` + `result = ExecutionResult::Ok` **without dispatching the recorded `ix_data` via CPI**. The handler comment notes "for M2 we record success — real CPI wiring lands with program-specific handlers." Off-chain consumers MUST NOT treat `ProposalExecuted{success:true}` as proof the target program changed; the `ExecutionRecord` PDA captures `cpi_target` + `cpi_payload_hash` as a record of *intent*, not on-chain execution. **Same disclosure shape as A.5.9's `finalize_batch` partial-implementation** — both are stub families whose output an off-chain consumer could misread as final.
- **Proposer-collateral slash on Rejected proposals.** `GovernanceConfig.proposer_collateral` is initialised at `init_config`; no current handler reads or transfers collateral. Spec reserves the surface; M2 scaffold expansion lands the deposit + slash-on-Rejected + return-on-Passed-or-Expired path.
- **Snapshot-freshness check.** `ProposalSnapshot.snapshot_slot` is recorded at `propose` time but not validated against `Clock::slot`. Spec §Snapshot-freshness window references a 100-slot bound; current handler accepts any slot. M1 reliance is on off-chain proposal-author honesty + ratifier review.
- **Forward-reserved metadata.** Several `RegisteredProgram` and `GovernanceConfig` fields (cross-program pubkey pins, per-program parameter-authority metadata) are stored but not read by the current scaffold; they document the M2 dispatch model rather than enforce it. The operational mechanism for M2 governance-driven parameter changes is direct authority rotation on the target program (the standard `transfer_authority` two-step), then governance-PDA-as-signer on the existing per-program setter ix surface — the on-chain CPI dispatch path is M2-scope.

#### A.6.9 Test coverage

**Rust unit + proptest (`cargo test -p governance_program`).** `src/state.rs` carries 3 inline unit tests (vote-window category dispatch, timelock critical-vs-standard-vs-Meta dispatch, single-leaf two-side merkle proof construction + verify against the canonical leaf-pair-then-hash construction). `src/fuzz.rs` carries 4 plain `#[test]` + 13 `proptest!` cases per A.6.7's discriminator-enforcement section. Default 256 invocations per proptest case.

**Anchor TS integration: not yet authored.** The closure path needs (a) a governance_program devnet deploy (currently not on mainnet per public release-status; M2-gated), (b) shared bankrun adapter coverage for the 7-day / 14-day / 21-day timelock warps + the 14-day execution window, (c) merkle-snapshot fixture (off-chain-computed root + leaf inclusion proof for the vote path). Same blocker class as the other three M2 programs (nxs_staking / fee_collector / dispute_arbitration) — coverage scaling waits on the M2 audit-prep window.

**Coverage gaps (known, not hidden).** No localnet integration test for the propose → vote → finalize → queue → execute lifecycle. No bankrun timelock adapter scaling for the three timelock cadences. No real CPI dispatch test (handler doesn't dispatch yet — A.6.8). No proposer-collateral lifecycle test (no handler enforces it yet — A.6.8). The §4.1 conditional-PASS rider applies — fuzz coverage is on the deserialisation layer only; the instruction layer (wrong-owner / missing-signer fuzz under SVM) is queued behind the `mollusk-svm` adoption that gates the same gap on every program in the appendix.

#### A.6.10 Finding-ledger filter

**Zero `F-2026-NN` findings touched governance_program.** The §5.1 ledger does not currently carry a governance_program row; the program was scaffolded inside the M2 work band, after the M1 finding stream that drove the F-2026-01..16 entries. Same posture as A.1.10 for capability_registry (zero findings, but for the opposite reason — A.1 was reviewed early and survived clean; A.6 has not yet been reviewed by an external party at all). The §1.2 sweep summarised in A.6.7 is internal-review-only; the M2 audit-prep window will bring its own finding stream.

**Conditional-PASS rider** for governance_program is **dual**: (a) the §A.6.8 CPI dispatch stub means the program does not yet exercise its primary role on-chain (no governance proposal has actually mutated a target program's state via this code path); (b) the program has not been read against §1.2 by an external reviewer. Both gaps close together when the M2 audit-prep window opens. Cross-cite to §6.6 for the inert-surface map (the `execute_handler` partial-implementation belongs alongside `finalize_batch` / `pay_task` / `record_job_outcome` in the audit-package's stub-family enumeration, even though governance_program post-dates the M1 audit-package).

### A.7 dispute_arbitration

One-sentence summary: dispute_arbitration is the M2 commit-reveal arbitration program that resolves task disputes raised through `task_market::raise_dispute`. It hosts a NXSStaking-rooted arbitrator pool, a weighted-VRF-driven random selection over the cumulative-stake walk, two rounds of commit-reveal voting (24h commit + 24h reveal each), an appeal-with-collateral escalation path, and a 30-day slash timelock with a 10% per-incident cap. **Posture: M2 scaffold — not yet initialised on mainnet** (see §2.1 row + mainnet phase 5 blocker on phases 1 + 3); 23 instructions, 9 `#[account]` PDAs, 20 events, 40 errors. **Zero `F-2026-NN` findings touched dispute_arbitration** at the time of this fold (see A.7.10).

#### A.7.1 Program identity

- **Program ID:** `GM8xiT17USBpCW24XXBmUR8YVCxxrJPMEcsddwfUokMa` (`programs/dispute_arbitration/src/lib.rs` `declare_id!`).
- **Spec:** [`specs/program-dispute-arbitration.md`](./specs/program-dispute-arbitration.md).
- **Mainnet status:** **not yet deployed.** §2.1's M2 row lists the program as scaffolded; mainnet phase 5 of the activation sprint blocks on phase 1 (governance_program init) + phase 3 (NXSStaking pool init for the arbitrator-stake source).
- **In-program `authority` (`DisputeConfig`):** deployer-supplied at `initialize`; the `pending_authority` field is reserved for a two-step rotation (no `propose/accept_authority` handler is implemented yet — flagged in A.7.8). Distinct from A.6: dispute_arbitration is the *callee* for M2 task settlement on `Disputed` tasks, not the destination authority for other programs' migrations.

#### A.7.2 PDAs

9 `#[account]` types in `state.rs`. All fixed-width `#[derive(InitSpace)]`; variable-length surfaces capped via `#[max_len(...)]`.

- **`DisputeConfig`** — singleton. Seeds `[b"dispute_config"]`. Carries `authority` + `pending_authority`; 6 cross-program pubkey pins (`task_market`, `nxs_staking`, `fee_collector`, `agent_registry`, `switchboard_program`, `emergency_council`); pool-sizing knobs (`round1_size = 3` / `round2_size = 5`, capped at `MAX_ROUND2_ARBITRATORS = 5`); commit/reveal/appeal windows (each 24h spec default); `appeal_collateral_bps = 15_000` (above 100% — appeal collateral exceeds the at-stake amount as a deterrent; `set_params` clamps to `<= 10_000` so widening past 100% requires a code change); `max_slash_bps = 1_000` (10% per-incident cap, validated `<= 10_000`); `slash_timelock_secs = 30 days`; arbitrator stake floor (`min_stake`, `min_lock_secs`); VRF freshness (`vrf_stale_slots = 150`); bad-faith strike tracking (`bad_faith_threshold = 3`, `bad_faith_lookback = 10`); monotonic `next_case_id`; `paused`; `bump`.
- **`ReentrancyGuard`** — singleton. Seeds `[b"guard"]`. Same shape as the four M1 programs that carry guard surface (A.2 / A.3 / A.4 / A.5): `active`, `entered_by`, `entered_at_slot`, `reset_proposed_at` (24h `ADMIN_RESET_TIMELOCK_SECS`), `bump`.
- **`AllowedCallers`** — singleton. Seeds `[b"allowed_callers"]`. `programs: Vec<Pubkey>` capped at `MAX_ALLOWED_CALLERS = 8`; `bump`.
- **`ArbitratorAccount`** — per-operator. Seeds `[b"arbitrator", operator]`. Carries `operator`, `stake_account` (NXSStaking PDA pin), `effective_stake`, `effective_lock_end`, `status: ArbitratorStatus` (4-state: Active / Paused / Slashed / Withdrawing), `bad_faith_strikes: u8`, `cases_participated: u32`, `withdraw_unlock_time`, `registered_at`, `bump`.
- **`DisputePool`** — singleton snapshot. Seeds `[b"dispute_pool"]`. Carries `snapshot_epoch`, `snapshot_time`, `total_staked: u128`, `arbitrator_count: u16`, `arbitrators: Vec<Pubkey>` capped at `MAX_POOL_SIZE = 256`, `cumulative_stakes: Vec<u64>` (running prefix sum used by `weighted_select`'s binary-search walk), `bump`.
- **`DisputeCase`** — per-case lifecycle. Seeds `[b"dispute_case", case_id_le_bytes]`. Carries `case_id`, `task_id`, `client`, `agent_operator`, `escrow_amount`, `payment_mint`, `status: DisputeStatus` (8-state: RequestedVrf / SelectionReady / Committing / Revealing / Tallied / Appealed / Resolved / Cancelled), `round: u8`, selected `arbitrators: Vec<Pubkey>` capped at `MAX_ROUND2_ARBITRATORS = 5`, `vrf_request`, `vrf_result: [u8; 32]`, `commit_deadline` + `reveal_deadline`, tally fields (`verdict: DisputeVerdict`, `votes_for_agent` / `votes_for_client` / `votes_for_split` all `u128`, `total_revealed_weight: u128`), `resolved_at`, `created_at`, `snapshot_pool` (the DisputePool key at raise-time), `bump`.
- **`DisputeVoteRecord`** — per-(case, arbitrator, round). Seeds `[b"dispute_vote", case_id_le_bytes, arbitrator, round_byte]`. Anchor-`init` semantics make double-vote per-round structurally impossible. Carries `case_id`, `arbitrator`, `round`, `commit_hash: [u8; 32]`, `committed_at`, `revealed_verdict: DisputeVerdict`, `revealed: bool`, `revealed_weight: u128`, `revealed_at`, `bump`.
- **`AppealRecord`** — per-case. Seeds `[b"appeal", case_id_le_bytes]`. Anchor-`init` semantics make double-appeal per-case structurally impossible. Carries `case_id`, `appellant`, `round = 2`, `collateral_amount`, `collateral_mint`, `filed_at`, `bump`.
- **`PendingSlash`** — per-arbitrator. Seeds `[b"pending_slash", arbitrator]`. Anchor-`init` semantics enforce single-outstanding-slash per arbitrator, mirroring the agent_registry pattern (A.2.6 invariant 10). Carries `arbitrator`, `case_id`, `amount`, `reason_code`, `executable_at`, `bump`.

#### A.7.3 Instructions

23 live public instructions grouped 6-way by concern. M2 cross-program CPIs are flagged inline as `// M2 structural` comments at the call sites where the next-revision wiring lands.

- **Init + guard mgmt** (5): `initialize(InitConfigParams)` (deployer-signed; creates `DisputeConfig` with hardcoded windows + thresholds + caps; caller-supplied `authority` / 5 cross-program pins / `min_stake` / `min_lock_secs` / `emergency_council`); `init_guard(initial_callers)` (`has_one = authority`; rejects defaulted pubkeys + `len > MAX_ALLOWED_CALLERS = 8`); `set_allowed_callers(programs)` (same gates); `propose_guard_reset()` (`has_one = authority`; sets `reset_proposed_at = now`); `admin_reset_guard()` (`has_one = authority`; enforces `now - reset_proposed_at >= ADMIN_RESET_TIMELOCK_SECS = 24h`).
- **Arbitrator lifecycle** (5): `register_arbitrator(effective_stake, lock_end)` (operator-signed; pause-gated; rejects below `min_stake` + below `min_lock_secs`; `stake_account` `owner = config.nxs_staking` checked structurally — actual NXSStaking PDA dereference is M2 CPI-deferred per the inline comment); `refresh_stake(new_stake, new_lock_end)` (operator-signed; caller-supplied at M2 scaffold; status auto-`Paused` if drops below `min_stake`); `snapshot_pool(arbitrators, stakes)` (`has_one = authority`; computes prefix-sum `cumulative_stakes` + total `u128`; rejects `MAX_POOL_SIZE = 256` overflow + length-mismatch; bumps `snapshot_epoch` via `wrapping_add`); `begin_withdraw()` (operator-signed; status must be Active or Paused; transitions to Withdrawing + sets `withdraw_unlock_time = now + round2_window_secs = 7d`); `complete_withdraw()` (operator-signed; status must be Withdrawing; rejects if `now < withdraw_unlock_time`; closes the `ArbitratorAccount` to the operator).
- **Dispute lifecycle** (3): `raise_dispute(task_id, client, agent_operator, escrow_amount, payment_mint)` (payer-signed at M2 scaffold — caller-identity validation is deferred to the M2 wiring per A.7.5; pause-gated; rejects when `pool.arbitrator_count < round1_size = 3`; `init`s `DisputeCase` at status `RequestedVrf`, `round = 1`; bumps `next_case_id` via `checked_add`); `consume_vrf(vrf_result)` (`has_one = authority` — Switchboard CPI is M2-deferred, current scaffold accepts the 32-byte VRF as an arg; rejects status ∉ `RequestedVrf | Appealed` + `pool.arbitrator_count < count`; calls `weighted_select` over `cumulative_stakes`; sets `commit_deadline = now + commit_window` + `reveal_deadline = now + commit + reveal`); `cancel_stale_vrf()` (cranker-signed; enforces `now > created_at + commit_window_secs` on a `RequestedVrf` case; transitions to Cancelled — TaskMarket force-release is M2 CPI-deferred).
- **Commit-reveal voting** (3): `commit_vote(commit_hash)` (arbitrator-signed; pause-gated; rejects status ≠ Committing + `now > commit_deadline` + signer ∉ `dc.arbitrators`; `init`s `DisputeVoteRecord` so double-commit per-round rejects via Anchor); `reveal_vote(verdict, salt)` (arbitrator-signed; rejects status ∉ `Committing | Revealing` + `now <= commit_deadline` (must be strictly past commit) + `now > reveal_deadline` + `verdict == None` + already-revealed + `compute_commit_hash(verdict, salt) != commit_hash`; transitions Committing → Revealing on first reveal; accumulates verdict-bucket vote weights via `checked_add`); `tally_round()` (cranker-signed; rejects status ≠ Revealing + `now <= reveal_deadline`; majority-test against `total_revealed_weight / 2`; on majority Revealing → Tallied; on no-majority round 1 auto-escalates to round 2 status Appealed; on no-majority round 2 records `verdict = None` + Tallied — fallback is authority review).
- **Resolution + appeal** (2): `escalate_appeal()` (appellant-signed; pause-gated; rejects status ≠ Tallied + `round != 1` + `now > reveal_deadline + appeal_window_secs` + appellant not the losing-party (validated against `dc.client` for AgentWins or `dc.agent_operator` for ClientWins); `init`s `AppealRecord` with `collateral_amount = escrow_amount * appeal_collateral_bps / 10_000`; transitions Tallied → Appealed + `round = 2`; collateral-lock CPI is M2 structural-deferred); `resolve_dispute()` (cranker-signed; rejects status ≠ Tallied; for round 1 enforces `now > reveal_deadline + appeal_window_secs` (no in-flight appeal possible); transitions Tallied → Resolved + sets `resolved_at`; CPI to `task_market::execute_dispute_verdict` + collateral-settlement is M2 structural-deferred).
- **Slashing + params** (5): `slash_arbitrator(reason_code)` (`has_one = authority`; rejects status ≠ Resolved on the case + already-pending slash via Anchor `init`; computes `slash_amount = min(stake * max_slash_bps / 10_000, stake)`; sets `executable_at = now + slash_timelock_secs = 30d`; arbitrator status flips to Paused + `bad_faith_strikes` saturating-add); `execute_slash()` (cranker-signed; rejects `now < executable_at`; M2 structural-deferred CPI to NXSStaking → fee_collector for the slashed amount; arbitrator promotes to Slashed if `bad_faith_strikes >= bad_faith_threshold = 3`, else back to Active; closes `PendingSlash` to cranker for rent refund); `cancel_slash()` (`has_one = authority`; rejects `now >= executable_at` (timelock-not-elapsed prevents post-window cancel); arbitrator status flips back to Active + saturating-sub on bad-faith strikes; closes `PendingSlash` to authority); `set_params(UpdateParamsInput)` (`has_one = authority`; per-field `Option<...>` shape — only set fields are touched; bps fields validated against `<= 10_000`); `set_paused(paused)` (authority OR `emergency_council` — explicit dual-path so the council can pause without round-tripping through the standard `authority`).

#### A.7.4 Events

20 events in `events.rs`, all decoded by `services/indexer` against the committed IDL: 5 guard-lifecycle (`GuardEntered`, `ReentrancyRejected`, `GuardInitialized`, `GuardAdminReset`, `AllowedCallersUpdated`); 2 arbitrator-lifecycle (`ArbitratorRegistered`, `PoolSnapshotted`); 6 dispute-lifecycle (`DisputeRaised`, `ArbitratorsSelected`, `DisputeCancelled`, `VoteCommitted`, `VoteRevealed`, `RoundTallied`); 2 resolution (`AppealEscalated`, `DisputeResolved`); 3 slashing (`SlashProposed`, `SlashExecuted`, `SlashCancelled`); 2 governance (`ParamsUpdated`, `PausedSet`). Coverage by the indexer's borsh round-trip harness is queued with the M2 indexer schema bump — same posture as A.6.4.

#### A.7.5 Cross-program consumption

**CPI out — none live in M2 scaffold.** Five CPI surfaces are spec'd and inline-commented as `// M2 structural` at the call sites: (1) `cancel_stale_vrf` → TaskMarket `force_release` to refund client on stale-VRF; (2) `consume_vrf` → Switchboard VRF program (current handler accepts the 32-byte result as an arg, no Switchboard CPI); (3) `escalate_appeal` collateral lock + `resolve_dispute` collateral settlement (Token-2022 `transfer_checked` on `payment_mint`); (4) `resolve_dispute` → `task_market::execute_dispute_verdict` to apply the verdict to the in-flight task; (5) `execute_slash` → NXSStaking → fee_collector for the slashed amount (mirrors the agent_registry slash routing in A.2.5 but on arbitrator stake). Off-chain consumers MUST NOT treat the current `DisputeResolved{verdict:X}` event as proof TaskMarket settled accordingly.

**CPI in — none live in M2 scaffold.** Three caller surfaces are spec'd: TaskMarket `raise_dispute` entry (the current scaffold's `raise_dispute` ix is payer-signed with no caller-identity check; the M2 wiring tightens to a CPI-only entry validated against `config.task_market`); NXSStaking `propose_slash` for arbitrator-bond slashing; GovernanceProgram `set_params` execution post-M2 authority migration.

**Inline cross-program reads — minimal at M2 scaffold.** `register_arbitrator` and `refresh_stake` validate `stake_account.owner == config.nxs_staking` via Anchor's `#[account(owner = ...)]` constraint, but the dereference of the NXSStaking PDA's `effective_stake` + `effective_lock_end` fields is M2-deferred — caller-supplied at handler entry, with the comment "M2 structural: effective_stake/lock_end passed as args until NXSStaking CPI reads land." A snapshot-honesty bond is not enforced at M2 scaffold (no on-chain reconciliation between `DisputePool.cumulative_stakes` and per-arbitrator `effective_stake` reads).

**Distinguishing CPI shape vs the prior folds:** A.1 (read-target only), A.2 (Token-2022 only + 1 CPI-in), A.3 (sole live SAEP-internal CPI-out), A.4 (3 CPI surfaces), A.5 (1 CPI-out + 1 CPI-in + 3 syscalls), A.6 (none-live, scaffold-stage). A.7 carries **none-live + 1 minimal inline cross-program owner-check + 5 forward-reserved M2 CPI surfaces** — the second-cleanest CPI shape after A.6, but with a richer structural spec for the M2 wiring than A.6 carries. Cycle-520 framing extends: A.7 joins A.6 in the M2 fold lane's "scaffold-stage rather than by design" posture.

#### A.7.6 Invariants

10 invariants: 7 from spec + 3 not-in-spec-but-in-code.

1. **Single dispute case per `case_id`.** Anchor-`init` on the seed-derived PDA structurally rejects re-init.
2. **Single vote per `(case, arbitrator, round)`.** Anchor-`init` on the seed-derived `DisputeVoteRecord` PDA — the round byte in the seed lets round 2 reuse the same arbitrator key without re-init collision.
3. **Single appeal per case.** Anchor-`init` on the seed-derived `AppealRecord` PDA. `round = 1` only — round-2 appeal is rejected via `TooManyAppeals`.
4. **Single outstanding slash per arbitrator.** Anchor-`init` on the seed-derived `PendingSlash` PDA — same pattern as A.2.6 invariant 10. `cancel_slash` and `execute_slash` both close the PDA (allowing re-propose post-execution).
5. **30-day slash timelock + 10% per-incident cap.** Hardcoded `DEFAULT_SLASH_TIMELOCK_SECS = 30d` + `DEFAULT_MAX_SLASH_BPS = 1_000` + `set_params` per-field validation against `<= 10_000` + `slash_amount = min(stake * bps / 10_000, stake)` clamp prevents over-slash even if a future `set_params` widens the bps. Same pattern as A.2.7 slashing-section.
6. **DisputeStatus is a one-way machine.** Legal transitions: RequestedVrf → {Committing | Cancelled}; Committing → Revealing; Revealing → Tallied; Tallied → {Appealed | Resolved}; Appealed → Committing (round 2 via `consume_vrf`). Each handler asserts the entry status; no handler accepts more than one prior status.
7. **`config.next_case_id` strictly monotonic.** `checked_add` fall-through to `ArithmeticOverflow`.
8. **Not in spec:** `commit_deadline` and `reveal_deadline` are stored at `consume_vrf` time, not recomputed. Subsequent `set_params` edits to `commit_window_secs` / `reveal_window_secs` do not retroactively shift in-flight deadlines.
9. **Not in spec:** Round 2 reuses the same `DisputeCase` PDA — only `round`, `arbitrators`, `commit_deadline`, `reveal_deadline`, vote tallies, and `verdict` are reset. Round-1 vote records remain on-chain for audit but are inaccessible to round 2 via the seed (round-byte distinguishes them).
10. **Not in spec:** Arbitrator-selection tie-break (when `weighted_select`'s seeded random index lands on an already-selected arbitrator) walks the `cumulative_stakes` vector forward until a fresh slot is found. Deterministic post-VRF (the walk is fully reproducible) but does shift marginal selection probability slightly toward higher-indexed arbitrators when the pool is small. Spec describes "weighted draw without replacement" without specifying the tie-break mechanism.

#### A.7.7 Security checks (§1.2 mapping)

- **Account validation.** All 9 `#[account]` PDAs declare `seeds = [...]` + `bump = <stored>.bump`. `DisputePool.arbitrators` and `cumulative_stakes` are `#[max_len(256)]`; `DisputeCase.arbitrators` is `#[max_len(5)]`; `AllowedCallers.programs` is `#[max_len(8)]`. `snapshot_pool` enforces `arbitrators.len() == stakes.len()` + `<= MAX_POOL_SIZE`. `consume_vrf` enforces `pool.arbitrator_count >= count` (round-sized) before the weighted-select walk.
- **Authorization.** `has_one = authority` on `init_guard`, `set_allowed_callers`, `propose_guard_reset`, `admin_reset_guard`, `consume_vrf`, `snapshot_pool`, `slash_arbitrator`, `cancel_slash`, `set_params`. `has_one = operator` on `refresh_stake`, `begin_withdraw`, `complete_withdraw`, `commit_vote`. `set_paused` is authority OR `emergency_council` — same dual-path as A.6.7. Permissionless cranks: `tally_round`, `cancel_stale_vrf`, `resolve_dispute`, `execute_slash` (all gated by status + time preconditions). Personhood gate: N/A — arbitrator gating is by `effective_stake >= min_stake` + `effective_lock_end >= now + min_lock_secs`, not by personhood tier.
- **Re-entrancy.** Same `ReentrancyGuard` + `AllowedCallers` shape as the M1 programs that carry guard surface. `MAX_CPI_STACK_HEIGHT = 3` const-only convention preserved (matches A.3.8 / A.4.8 / A.5.8); cross-program parity callout vs A.2.7's `<= 2` literal cross-cited via §3.2 + §6.2. `check_callee_preconditions` enforces the 4-tuple (stack-height + caller in allowlist + caller-guard active + self-guard not active). M2 CPI dispatch wiring is the gating step that turns enforcement live — current scaffold initialises the surface but no CPI-in path actually invokes `try_enter` yet (joining A.6 on enforcement-deferred posture, the M1 set on structural shape).
- **Integer safety.** `checked_add` on `next_case_id`, on every vote-weight accumulation (`votes_for_agent` / `votes_for_client` / `votes_for_split` / `total_revealed_weight` `u128`), on `commit_deadline` / `reveal_deadline` / `appeal_deadline` / `executable_at` / `withdraw_unlock_time` / `stale_after` `i64` time deltas, on `cumulative` running sums in `snapshot_pool`. `checked_mul` on the `escrow * appeal_collateral_bps` and `stake * max_slash_bps` ratio computations (`u128` intermediate). `bad_faith_strikes` uses `saturating_add` / `saturating_sub` (intentional — strike count is bounded `0..=255`). `min(slash_amount, effective_stake)` clamp post-bps prevents over-slash. Uniform `ArithmeticOverflow` error.
- **Upgrade.** Deployer → Squads vault with timelock per ops spec before mainnet. The `authority` field on `DisputeConfig` is the program's *internal* authority for `set_params` / `set_allowed_callers` / `slash_arbitrator` / `consume_vrf`. The `pending_authority` field exists but no `propose_authority_transfer` / `accept_authority` ix is implemented — flagged in A.7.8.
- **Pause.** `config.paused` checked on `register_arbitrator`, `raise_dispute`, `commit_vote`, `escalate_appeal`. `reveal_vote` / `tally_round` / `consume_vrf` / `cancel_stale_vrf` / `resolve_dispute` / `slash_arbitrator` / `execute_slash` / `cancel_slash` / `begin_withdraw` / `complete_withdraw` deliberately bypass pause so in-flight cases can complete their lifecycle — same exit-path-liveness rationale as treasury_standard (A.4.8) and governance_program (A.6.7). `set_paused` itself is authority OR `emergency_council`.
- **Slashing.** 30-day default timelock (governance-adjustable via `set_params`, no minimum bound enforced — flagged in A.7.8), 10% per-incident cap, single-outstanding rule (Invariant 4), `cancel_slash` allowed only pre-window (`now < executable_at`). The slash-amount-min clamp against `effective_stake` prevents over-slash even if a future `set_params` widens `max_slash_bps`. Same shape as A.2.7's slashing section but on arbitrator stake instead of agent stake. Bad-faith strike accumulation auto-promotes the arbitrator from Paused → Slashed at `>= bad_faith_threshold = 3` strikes.
- **Token-2022.** N/A at M2 scaffold — dispute_arbitration holds no token accounts. `payment_mint` is recorded on `DisputeCase` for off-chain reconciliation; the appeal-collateral lock + slash routing are M2 CPI-deferred (A.7.5).
- **Oracle.** N/A — dispute_arbitration performs no oracle reads. VRF-result freshness is window-gated via `cancel_stale_vrf`'s `now > created_at + commit_window_secs` check, not per-slot oracle freshness.
- **PDA spoofing.** Stored-bump pattern on all 9 PDAs. No `find_program_address` at runtime in the handlers.
- **Discriminator enforcement.** `fuzz.rs` covers all 9 `#[account]` types: 9 plain `#[test]` (round-trip on `DisputeConfig` / `ReentrancyGuard` / `AllowedCallers`, truncated-to-discriminator rejection across the 3 primary shapes, empty-buffer rejection, pairwise-distinct discriminators across all 9 PDAs, commit-hash determinism + verdict-distinguishment + salt-distinguishment) plus 9 `proptest!` cases (arbitrary-bytes-discriminator rejection on `DisputeConfig` / `ReentrancyGuard`, arbitrary-bytes per-PDA on `ArbitratorAccount` / `DisputeCase` / `DisputeVoteRecord` / `AppealRecord` / `PendingSlash`, commit-hash no-collision, guard-roundtrip-random). Default 256 invocations per proptest case → ~2,304 randomised runs against the deserialisation layer. The §4 matrix counts `dispute_arbitration | 19 cases`; this fold counts 18 (9 plain + 9 proptest, per current source) — counting-convention discrepancy explicitly disclosed.

#### A.7.8 Known stubs

- **Five M2-structural CPI sites.** Each carries a `// M2 structural` comment in `instructions/*.rs`. Off-chain consumers MUST NOT treat the M2-scaffold lifecycle events as proof of cross-program effect — see A.7.5 for the full enumeration. **Same disclosure shape as A.6.8's `execute_handler` partial-implementation** — both M2 programs land their primary-role on-chain CPI dispatch at the M2 audit-prep window.
- **Switchboard VRF integration.** `consume_vrf` accepts the 32-byte VRF result as a handler arg, with `has_one = authority` gating who can submit it. Spec describes a Switchboard CPI that pulls the result on-chain; current scaffold trusts the authority-supplied result.
- **NXSStaking stake-integrity reads.** `register_arbitrator` and `refresh_stake` accept `effective_stake` and `effective_lock_end` as handler args, validated only against `>= min_stake` and `>= now + min_lock_secs`. The on-chain dereference of NXSStaking PDA fields is M2-deferred. M2-scaffold reliance is on operator honesty + off-chain ratifier review.
- **Authority rotation ix surface.** `DisputeConfig.pending_authority` exists but no `propose_authority_transfer` / `accept_authority` handlers are implemented; current scaffold sets `pending_authority = Pubkey::default()` at init and never touches it. M2 scaffold expansion lands the standard two-step transfer pattern (mirroring the M1 programs).
- **Snapshot-honesty enforcement.** `snapshot_pool` accepts `arbitrators` and `stakes` from `has_one = authority` without on-chain reconciliation against per-arbitrator `ArbitratorAccount.effective_stake`. M2 lands either an authority-signed honesty bond or a per-arbitrator inclusion-proof check.
- **Caller-identity check on `raise_dispute`.** Current handler is payer-signed with no `caller_program_id` check against `config.task_market`. M2 wiring tightens to a CPI-only entry per A.7.5.

#### A.7.9 Test coverage

**Rust unit + proptest (`cargo test -p dispute_arbitration`).** `src/fuzz.rs` carries 9 plain `#[test]` + 9 `proptest!` cases per A.7.7's discriminator-enforcement section. `src/guard.rs` carries 6 inline unit tests (enter/exit, double-enter rejection, callee-rejects-random-program, callee-rejects-deep-stack, callee-rejects-inactive-caller-guard, timelock-enforced — same shape as the M1 programs' `guard.rs` unit tests). Default 256 invocations per proptest case.

**Anchor TS integration: not yet authored.** The closure path needs (a) a dispute_arbitration devnet deploy (mainnet phase 5 blocked on phases 1 + 3); (b) shared bankrun adapter coverage for the 30-day slash timelock + the 24h commit/reveal/appeal windows + the 24h `ADMIN_RESET_TIMELOCK_SECS`; (c) NXSStaking devnet deploy + arbitrator-stake fixture; (d) Switchboard VRF fixture. Same blocker class as the other three M2 programs (governance_program / nxs_staking / fee_collector) — coverage scaling waits on the M2 audit-prep window.

**Coverage gaps (known, not hidden).** No localnet integration test for the raise → consume_vrf → commit-reveal × 2 rounds → resolve lifecycle. No bankrun timelock adapter for the 30-day slash + 24h windows. No real CPI dispatch test (no live CPI yet — A.7.5 + A.7.8). No appeal-collateral or slash-CPI flow test (M2-deferred). No VRF-consumption test against a real Switchboard fixture. The §4.1 conditional-PASS rider applies — fuzz coverage is on the deserialisation layer only; the instruction layer (wrong-owner / missing-signer fuzz under SVM) is queued behind the `mollusk-svm` adoption that gates the same gap on every program in the appendix.

#### A.7.10 Finding-ledger filter

**Zero `F-2026-NN` findings touched dispute_arbitration.** The §5.1 ledger does not currently carry a dispute_arbitration row; the program was scaffolded inside the M2 work band, after the M1 finding stream that drove the F-2026-01..16 entries. Same posture as A.6.10 for governance_program (zero findings, scaffold-stage M2). The §1.2 sweep summarised in A.7.7 is internal-review-only; the M2 audit-prep window will bring its own finding stream.

**Conditional-PASS rider** for dispute_arbitration is **dual**: (a) the §A.7.5 + §A.7.8 M2-structural CPI gaps mean the program does not yet exercise its primary roles on-chain (no real dispute has been resolved through this code path against a live TaskMarket case); (b) the program has not been read against §1.2 by an external reviewer. Both gaps close together when the M2 audit-prep window opens. Cross-cite to §6.6 for the inert-surface map (the 5 M2-structural CPI gaps belong alongside `finalize_batch` / `pay_task` / `record_job_outcome` / `execute_handler` in the audit-package's stub-family enumeration, even though dispute_arbitration post-dates the M1 audit-package).

### A.8 nxs_staking

One-sentence summary: nxs_staking is the M2 SAEP-token staking program that hosts lockup-weighted stake accounts (1x–4x voting-power multiplier on a 7d–365d lockup), a 3-day unstake cooldown, an authority-supplied per-epoch voting-power snapshot rail, and a 180-day migration-window grace for pool closure ahead of the M3 mint hand-off. **Posture: M2 scaffold — deployed-but-inert on mainnet** at `GjXfJ6MHb6SJ4XBK3qcpGw4n256qYPrDcXrNj6kf2i2Z` (last upgrade slot 414799704; pool not yet initialised, no stake accounts exist on-chain — see §2.1 row + mainnet phase 3 of the activation sprint); 14 instructions, 6 `#[account]` PDAs (3 staking-side + 3 guard-side), 14 events, 21 errors. **Zero `F-2026-NN` findings touched nxs_staking** at the time of this fold (see A.8.10).

#### A.8.1 Program identity

- **Program ID:** `GjXfJ6MHb6SJ4XBK3qcpGw4n256qYPrDcXrNj6kf2i2Z` (`programs/nxs_staking/src/lib.rs` `declare_id!`).
- **Spec:** [`specs/program-nxs-staking.md`](./specs/program-nxs-staking.md).
- **Mainnet status:** **deployed but inert** — distinct from A.6 (governance_program, not yet deployed) and A.7 (dispute_arbitration, not yet deployed). The program account is on mainnet at the named program ID but the `StakingPool` PDA at seeds `[b"staking_pool"]` is not yet initialised; mainnet phase 3 of the activation sprint runs `init_pool` + the rewards loop after phase 1's governance_program init lands.
- **In-program `authority` (`StakingConfig`):** deployer-supplied at `initialize`. The `StakingPool.pending_authority` field exists as `Option<Pubkey>` but no `propose_authority_transfer` / `accept_authority` ix is implemented yet — flagged in A.8.8.

#### A.8.2 PDAs

6 `#[account]` types — 3 in `state.rs` (staking-side) + 3 in `guard.rs` (guard-side). All fixed-width `#[derive(InitSpace)]`; the only variable-length surface is `AllowedCallers.programs` capped via `#[max_len(8)]`.

- **`StakingConfig`** — singleton. Seeds `[b"staking_config"]`. Carries `authority` + `bump`. Distinct from `StakingPool` — `StakingConfig` gates the guard-admin surface (`init_guard` / `set_allowed_callers` / `propose_guard_reset` / `admin_reset_guard` / `migrate_apy_authority` / `snapshot_epoch`); `StakingPool` gates the staking surface.
- **`ReentrancyGuard`** — singleton. Seeds `[b"guard"]`. Same shape as the M1 programs that carry guard surface (A.2 / A.3 / A.4 / A.5) and as A.7's dispute_arbitration: `active`, `entered_by`, `entered_at_slot`, `reset_proposed_at` (24h `ADMIN_RESET_TIMELOCK_SECS`), `bump`.
- **`AllowedCallers`** — singleton. Seeds `[b"allowed_callers"]`. `programs: Vec<Pubkey>` capped at `MAX_ALLOWED_CALLERS = 8`; `bump`.
- **`StakingPool`** — singleton. Seeds `[b"staking_pool"]`. Carries `authority` + `pending_authority: Option<Pubkey>`; `stake_mint` (the SAEP mint at M3, placeholder mint at M2); aggregate counters (`total_staked: u64`, `total_stakers: u32`); epoch state (`current_epoch`, `epoch_duration_secs`, `epoch_start_time`, `reward_rate_per_epoch`); 3 pause flags (`paused` for full-stop, `pause_new_stakes` + `pause_new_stakes_at` for the migration-window deposit freeze, `closed` + `closed_at` for terminal pool closure); `bump`.
- **`StakeAccount`** — per-(pool, owner). Seeds `[b"stake", pool.key(), owner.key()]`. **Single stake per owner per pool** — distinct from the spec's `(operator, lock_id)` multi-lock pattern (A.8.6 invariant 7 not-in-spec). Carries `owner`, `pool`, `amount: u64`, `lockup_end: i64`, `lockup_multiplier: u8`, `voting_power: u64` (cached `amount * multiplier`), `staked_at`, `cooldown_start`, `pending_rewards: u64` (forward-reserved — no reward-claim ix lands at M2), `last_claim_epoch`, `status: StakeStatus` (3-state: Active / Cooldown / Withdrawn), `bump`, `vault_bump`.
- **`VotingPowerSnapshot`** — per-epoch. Seeds `[b"epoch_snapshot", current_epoch_le_bytes]`. Carries `epoch`, `total_voting_power: u128`, `snapshot_time`, `staker_count: u32`, `bump`.
- **No `PendingSlash` PDA** — the spec's 30d slash timelock + 10% cap surface is **not yet landed in code**. The §4 fuzz matrix line 179 lists `PendingSlash` as an in-scope account; this fold counts against the actual current source which carries no slashing path. Cross-cited in A.8.8.

#### A.8.3 Instructions

14 live public instructions grouped 4-way by concern. No stubs whose output a caller could misread as final (distinct from A.5.9's `finalize_batch`, A.6.8's `execute_handler`, and A.7.8's 5 M2-structural CPI sites — nxs_staking's M2 deferrals are entirely *missing* surface, not partial-implementation surface that emits success events).

- **Init + guard mgmt** (5): `initialize(authority)` (deployer-signed; creates `StakingConfig`); `init_guard(initial_callers)` (`has_one = authority`; rejects `len > MAX_ALLOWED_CALLERS = 8` + defaulted pubkeys); `set_allowed_callers(programs)` (same gates); `propose_guard_reset()` (`has_one = authority`; sets `reset_proposed_at = now`); `admin_reset_guard()` (`has_one = authority`; enforces `now - reset_proposed_at >= ADMIN_RESET_TIMELOCK_SECS = 24h`).
- **Pool init** (1): `init_pool(stake_mint, epoch_duration_secs, reward_rate_per_epoch)` (`has_one = authority` against `StakingConfig`; creates `StakingPool` at fixed seed `[b"staking_pool"]`; sets `epoch_start_time = now`, `current_epoch = 0`, all pause flags false).
- **Staking lifecycle** (3): `stake(amount, lockup_duration_secs)` (owner-signed; rejects `paused` + `closed` + `pause_new_stakes` + `amount == 0` + `lockup_duration_secs ∉ [MIN_LOCKUP_SECS = 7d, MAX_LOCKUP_SECS = 365d]`; computes `multiplier = compute_multiplier(lockup_duration_secs)` (1× at 7d → 4× at 365d, linear in `(elapsed / range)` with `MAX_VOTING_POWER_MULTIPLIER = 4`); computes `voting_power = compute_voting_power(amount, multiplier)` (saturating `u64::MAX` clamp via `u128` intermediate); Token-2022 `transfer_checked` from `owner_token_account` to PDA-signed `vault` at `seeds = [b"stake_vault", stake_account.key()]`; `init`s the `StakeAccount` so re-stake by the same owner before `withdraw` rejects via Anchor); `begin_unstake()` (`has_one = owner`; rejects `closed` + status ≠ Active + `now < lockup_end`; transitions Active → Cooldown + sets `cooldown_start = now`; `total_staked` debited via `saturating_sub`); `withdraw()` (`has_one = owner`; rejects status ≠ Cooldown + `now < cooldown_start + COOLDOWN_SECS = 3d`; PDA-signed `transfer_checked` from `vault` back to `owner_token_account`; transitions Cooldown → Withdrawn + zeros `amount` + `voting_power`; `total_stakers` debited via `saturating_sub`).
- **Pool ops** (5): `freeze_deposits()` (`has_one = authority` against `StakingPool`; rejects `pause_new_stakes` already-true; sets `pause_new_stakes_at = now` — anchors the 180-day migration grace clock); `unfreeze_deposits()` (same gate; rejects `pause_new_stakes` already-false); `close_pool()` (same gate; rejects `closed` already-true; **terminal precondition**: `pool.total_staked == 0` OR (`pause_new_stakes` true AND `now >= pause_new_stakes_at + MIGRATION_WINDOW_SECS = 180d`); transitions to `closed` + sets `closed_at`); `migrate_apy_authority(old_mint, new_mint)` (`has_one = authority` against `StakingConfig`; **event-only handler at M2** — emits `ApyAuthorityMigrated{old_mint, new_mint, attested_at}` without any Token-2022 `set_authority` CPI; the `InterestBearing` apy-authority CPI is M3-deferred per `specs/nxs-m3-migration.md` and lives on the SAEP mint at M3, not on the placeholder mint at M2); `snapshot_epoch(total_voting_power, staker_count)` (`has_one = authority` against `StakingConfig`; rejects `closed` + `now < epoch_start_time + epoch_duration_secs`; `init`s `VotingPowerSnapshot` at the current `epoch` seed so re-snapshot of the same epoch rejects via Anchor; bumps `current_epoch` + resets `epoch_start_time = now`).

#### A.8.4 Events

14 events in `events.rs`, all decoded by `services/indexer` against the committed IDL: 5 guard-lifecycle (`GuardEntered`, `ReentrancyRejected`, `GuardInitialized`, `GuardAdminReset`, `AllowedCallersUpdated`); 4 staking-lifecycle (`PoolInitialized`, `Staked`, `UnstakeInitiated`, `Withdrawn`); 1 epoch (`EpochSnapshotted`); 4 pool-ops (`DepositsFrozen`, `DepositsUnfrozen`, `PoolClosed`, `ApyAuthorityMigrated`). Coverage by the indexer's borsh round-trip harness is queued with the M2 indexer schema bump — same posture as A.6.4 and A.7.4.

#### A.8.5 Cross-program consumption

**CPI out — Token-2022 only (2 sites).** `stake` runs `transfer_checked(owner → vault, amount, decimals)` with the owner as authority (signer-authority pattern, no PDA signing); `withdraw` runs `transfer_checked(vault → owner, amount, decimals)` with the `vault` PDA as authority (PDA-signed via `seeds = [b"stake_vault", stake_account.key(), vault_bump]`). Both sites use `anchor_spl::token_interface::TransferChecked` against `Interface<TokenInterface>` so SPL-Token + Token-2022 are accepted symmetrically. No `inspect_mint_extensions` checkpoint at M2 — the placeholder mint carries no mint-extension drift surface; the M3 hand-off lands the inspection in step with the SAEP mint's `TransferHook` / `PermanentDelegate` / `InterestBearing` activation per `specs/token2022-saep-mint.md` (the spec is currently aspirational pending the INBOX-line-21 confidential-transfer-direction ratification — A.8 is structurally independent of that ratification because nxs_staking holds no mint-extension authorities at M2).

**CPI in — none live in M2 scaffold.** Three caller surfaces are spec'd: GovernanceProgram `verify_snapshot_root` for vote-weight binding (A.6.5 cross-cite — currently `total_voting_power` is authority-supplied at `snapshot_epoch`, not merkle-rooted from per-stake reads); AgentRegistry `propose_slash` for agent-stake slashing (A.2.5 cross-cite — current AgentRegistry slash routing terminates at fee_collector NXS_ATA, not at NXSStaking); DisputeArbitration `propose_slash` for arbitrator-stake slashing (A.7.5 cross-cite — `execute_slash` is M2-deferred there).

**Inline cross-program reads — none.** The `stake_mint` field on `StakingPool` is validated via Anchor `#[account(address = pool.stake_mint)]`, not via cross-program PDA dereference. No SAEP-internal program field is read from any handler.

**Distinguishing CPI shape vs the prior folds:** A.1 (read-target only), A.2 (Token-2022 only + 1 CPI-in), A.3 (sole live SAEP-internal CPI-out), A.4 (3 CPI surfaces), A.5 (1 CPI-out + 1 CPI-in + 3 syscalls), A.6 (none-live + 4 forward-reserved pubkey pins), A.7 (none-live + 1 minimal owner-check + 5 forward-reserved). A.8 carries **Token-2022 only (2 sites) + none-in + zero forward-reserved cross-program pins** — closest in shape to A.2 but smaller (2 sites vs 4) and without A.2's CPI-in (`update_reputation` from proof_verifier). Cleanest of the live Token-2022 footprints in the appendix.

#### A.8.6 Invariants

8 invariants: 5 from spec + 3 not-in-spec-but-in-code.

1. **Single stake per `(pool, owner)`.** Anchor-`init` on the seed-derived `StakeAccount` PDA structurally rejects re-stake by the same owner before `withdraw` clears the PDA.
2. **Single epoch snapshot per `(pool, current_epoch)`.** Anchor-`init` on the seed-derived `VotingPowerSnapshot` PDA — re-snapshot of the same epoch rejects via Anchor; `current_epoch` is incremented post-snapshot.
3. **`StakeStatus` is a one-way machine.** Legal transitions: Active → Cooldown (only after `lockup_end`); Cooldown → Withdrawn (only after `cooldown_start + COOLDOWN_SECS = 3d`). Each handler asserts the entry status; no handler accepts more than one prior status.
4. **`pool.current_epoch` strictly monotonic.** `checked_add` fall-through to `ArithmeticOverflow` at every `snapshot_epoch` increment.
5. **`pool.total_staked` debited at `begin_unstake`, not at `withdraw`.** The lockup-window stake counts toward `total_staked`; the cooldown-window stake does not. Surfaced explicitly because off-chain dashboards reading `total_staked` see the cooldown-window dip 3 days before the actual `withdraw` lands.
6. **Not in spec:** `lockup_end` is stored at `stake` time, not recomputed. Subsequent `stake` calls by other owners that change the pool's reward rate or epoch cadence do not retroactively shift any in-flight stake's lockup window.
7. **Not in spec:** Single-stake-per-owner per pool (A.8.2 callout). The spec describes a `(operator, lock_id)` multi-lock surface; the current scaffold's seed pattern `[b"stake", pool, owner]` enforces one-stake-per-owner. Surfaced explicitly because off-chain consumers that expect the multi-lock surface will mis-shape their indexes against the current scaffold.
8. **Not in spec:** `migrate_apy_authority` is a no-op event emit at M2. The handler accepts `(old_mint, new_mint)` and emits `ApyAuthorityMigrated{old_mint, new_mint, attested_at}` without any on-chain authority transfer. M3 lands the actual `set_authority` CPI against the SAEP mint's `InterestBearing` extension per `specs/nxs-m3-migration.md`. Surfaced explicitly because the event emit could be misread as proof the apy-authority moved.

#### A.8.7 Security checks (§1.2 mapping)

- **Account validation.** All 6 `#[account]` PDAs declare `seeds = [...]` + `bump = <stored>.bump`. `AllowedCallers.programs` is `#[max_len(8)]`. `stake` enforces `lockup_duration_secs ∈ [MIN_LOCKUP_SECS = 7d, MAX_LOCKUP_SECS = 365d]` via `RangeInclusive::contains` + `amount > 0`. `close_pool` enforces the dual-precondition (`total_staked == 0` OR migration-window-elapsed) so the authority cannot close a pool that still has stakers without the 180-day grace.
- **Authorization.** `has_one = authority` on `init_guard` / `set_allowed_callers` / `propose_guard_reset` / `admin_reset_guard` / `migrate_apy_authority` / `snapshot_epoch` (against `StakingConfig`) and on `init_pool` / `freeze_deposits` / `unfreeze_deposits` / `close_pool` (against `StakingPool`). `has_one = owner` on `begin_unstake` / `withdraw` (against `StakeAccount`). Permissionless: `stake` (any signer creates their own `StakeAccount`). No emergency-council dual-path on `set_paused` — distinct from A.6.7 and A.7.7 (current scaffold has no `set_paused` handler at all; the `pool.paused` flag is set by the authority via `freeze_deposits` + `pause_new_stakes` only).
- **Re-entrancy.** Same `ReentrancyGuard` + `AllowedCallers` shape as the M1 programs that carry guard surface and as A.7. `MAX_CPI_STACK_HEIGHT = 3` const-only convention preserved (matches A.3.8 / A.4.8 / A.5.8 / A.7.7); cross-program parity callout vs A.2.7's `<= 2` literal cross-cited via §3.2 + §6.2. `check_callee_preconditions` enforces the 4-tuple (stack-height + caller in allowlist + caller-guard active + self-guard not active). No CPI-in path actually invokes `try_enter` yet at M2 scaffold — same enforcement-deferred posture as A.6 and A.7.
- **Integer safety.** `checked_add` on `pool.total_staked` accumulation, on `current_epoch` increment, on `lockup_end = now + lockup_duration_secs`, on `cooldown_end = now + COOLDOWN_SECS`, on `epoch_end = epoch_start_time + epoch_duration_secs`, on `pause_new_stakes_at + MIGRATION_WINDOW_SECS`. `saturating_sub` on `pool.total_staked` debit at `begin_unstake` (not `checked_sub` — bounded `0..=total_staked` so saturation is safe; surfaced explicitly because the non-checked variant is non-default in the SAEP codebase). `saturating_add` on `pool.total_stakers` increment + `saturating_sub` on decrement (bounded `0..=u32::MAX`). `compute_voting_power` uses `u128` intermediate with `min(u64::MAX as u128)` clamp to prevent overflow at `amount * 4`. Uniform `ArithmeticOverflow` error.
- **Upgrade.** Deployer → Squads vault with timelock per ops spec before mainnet phase 3. The `authority` field on `StakingConfig` is the program's *internal* authority for the guard-admin surface; `pending_authority: Option<Pubkey>` exists on `StakingPool` for a two-step pool-authority rotation but no handler implements the propose / accept pattern — flagged in A.8.8.
- **Pause.** Three pause flags on `StakingPool`: `paused` (full-stop, only checked on `stake`), `pause_new_stakes` (deposit-freeze for migration window, only checked on `stake`), `closed` (terminal, checked on `stake` + `begin_unstake` + `snapshot_epoch`). `withdraw` deliberately bypasses all three so cooldown-window unstakes can complete even after pool closure — same exit-path-liveness rationale as treasury_standard A.4.8 and governance_program A.6.7. No `set_paused` ix; `paused` is set at init only and is currently dormant — the operational pause path is `freeze_deposits`, not the `paused` flag.
- **Slashing.** **N/A in M2 scaffold** — no `propose_slash` / `execute_slash` / `cancel_slash` handlers and no `PendingSlash` PDA. The §4 fuzz matrix's PDA list (`StakingConfig + StakeAccount + PendingSlash + SnapshotAccount`) is spec-time, not source-current. Spec describes a 30d uniform timelock + 10% per-incident cap + max-3-concurrent slashes + CPI-only entry from 3 hard-pinned slashers (AgentRegistry / DisputeArbitration / GovernanceProgram); current scaffold lands none of it. M2 expansion lands the surface (see A.8.8).
- **Token-2022.** 2 `transfer_checked` sites at `stake` (deposit) + `withdraw` (refund-from-vault), both via `Interface<TokenInterface>` so SPL-Token + Token-2022 are accepted symmetrically. No `inspect_mint_extensions` checkpoint at M2 — the placeholder mint carries no mint-extension drift surface (no `TransferHook` / `PermanentDelegate` / `InterestBearing` set on the placeholder; cross-cite to A.4.6's mint-extension verification pattern for the M3 hand-off).
- **Oracle.** N/A — nxs_staking performs no oracle reads. `compute_multiplier`'s lockup-duration-to-multiplier curve is a closed-form computation, not an oracle-dependent rate.
- **PDA spoofing.** Stored-bump pattern on all 6 PDAs. No `find_program_address` at runtime in the handlers. The `stake_vault` PDA's bump is stored on the parent `StakeAccount.vault_bump` rather than being re-derived — preserves PDA-signing determinism across handlers without re-paying the `find_program_address` cost on `withdraw`.
- **Discriminator enforcement.** `fuzz.rs` covers all 6 `#[account]` types: 7 happy-path / shape-distinguishing `proptest!` (`staking_config_roundtrip`, `reentrancy_guard_roundtrip`, `allowed_callers_roundtrip`, `staking_pool_roundtrip`, `multiplier_monotonic`, `multiplier_always_in_range`, `voting_power_no_overflow`) plus 7 negative / capacity `proptest!` (`arbitrary_bytes_staking_config`, `arbitrary_bytes_reentrancy_guard`, `arbitrary_bytes_allowed_callers`, `arbitrary_bytes_staking_pool`, `arbitrary_bytes_stake_account`, `arbitrary_bytes_voting_power_snapshot`, `oversized_caller_list_rejected`). Default 512 invocations per case (raised from the 256-default in the M1 programs and A.6 / A.7 — `cases: 512` set at the top of the proptest! block) → ~7,168 randomised runs against the deserialisation layer. The §4 matrix counts `nxs_staking | 15 cases`; this fold counts 14 (per current source) — counting-convention discrepancy explicitly disclosed (same shape as A.6.7's 18-vs-17 and A.7.7's 19-vs-18 disclosures).

#### A.8.8 Known stubs

- **No slashing surface.** Spec describes 16 instructions including `propose_slash` / `execute_slash` / `cancel_slash` + a `PendingSlash` PDA + a 30d timelock + a 10% per-incident cap + a max-3-concurrent rule + CPI-only entry from 3 hard-pinned slashers. Current scaffold lands 14 instructions and 6 PDAs; the slashing surface is **not yet landed**. Off-chain consumers MUST NOT treat the absence of slashing as proof stake is unslashable — the M2 audit-prep window lands the surface, and the `authority` field on `StakingConfig` migrates to the GovernanceProgram PDA so the slashing-CPI entry is governance-bound.
- **`migrate_apy_authority` is event-only.** The handler emits `ApyAuthorityMigrated{old_mint, new_mint, attested_at}` without any on-chain authority transfer. `specs/nxs-m3-migration.md` lands the actual M3 migration design (4 ix surface: `freeze_deposits` / `close_pool` / `set_apy` / `migrate_apy_authority` against the SAEP mint's `InterestBearing` extension via a singleton `apy_authority` PDA). M2 scaffold's reliance is on off-chain attestation that the migration happened; M3 lands the on-chain `set_authority` CPI. **Same disclosure shape as A.6.8's `execute_handler` partial-implementation** — both are stubs whose success-event an off-chain consumer could misread as proof of cross-program effect.
- **Snapshot-honesty enforcement.** `snapshot_epoch` accepts `total_voting_power` and `staker_count` from `has_one = authority` without on-chain reconciliation against a per-`StakeAccount` walk. M2 lands either an authority-signed honesty bond or a per-stake inclusion-proof check (the spec describes a 100-slot freshness window + depth-24 merkle tree; current scaffold has neither). M2-scaffold reliance is on authority honesty + off-chain ratifier review.
- **Authority rotation ix surface.** `StakingPool.pending_authority: Option<Pubkey>` exists but no `propose_authority_transfer` / `accept_authority` handlers are implemented; current scaffold sets `pending_authority = None` at init and never touches it. M2 scaffold expansion lands the standard two-step transfer pattern (mirroring the M1 programs).
- **Reward-claim path.** `StakeAccount.pending_rewards: u64` and `last_claim_epoch: u64` are forward-reserved fields — no `claim_rewards` ix is implemented at M2. The spec describes a fee_collector `commit_distribution_root` + per-staker merkle-claim cycle; current scaffold has neither the on-chain claim path nor the merkle-root commit. M2 audit-prep window lands the surface in step with fee_collector's epoch distribution rail.
- **Multiple-concurrent-locks per operator.** Spec describes `(operator, lock_id)` PDA seeding for multiple concurrent stakes per operator; current scaffold's `[b"stake", pool, owner]` seed pattern enforces one-stake-per-owner per pool (A.8.6 invariant 7 not-in-spec). M2 expansion either lands the multi-lock seed change (with a migration plan for any in-flight M2 stakes) or the spec is rewritten to match the single-stake reality.

#### A.8.9 Test coverage

**Rust unit + proptest (`cargo test -p nxs_staking`).** `src/state.rs` carries 2 inline unit tests (`multiplier_bounds` + `voting_power_math` — bounds + saturation). `src/guard.rs` carries 6 inline unit tests (`enter_exit`, `double_enter_rejected`, `callee_rejects_random_program`, `callee_rejects_deep_stack`, `callee_rejects_inactive_caller_guard`, `timelock_enforced`) — same shape as the M1 programs' `guard.rs` unit tests. `src/fuzz.rs` carries 14 `proptest!` cases per A.8.7's discriminator-enforcement section. 512 invocations per proptest case (intentionally raised from the 256-default elsewhere in the appendix).

**Anchor TS integration: not yet authored.** The closure path needs (a) an nxs_staking devnet deploy + `init_pool` against the placeholder mint (currently deployed-but-inert on mainnet per A.8.1), (b) shared bankrun adapter coverage for the 7d–365d lockup warps + the 3d cooldown + the 180d migration window, (c) a fee_collector reward-distribution fixture once the claim path lands. Same blocker class as the other three M2 programs (governance_program / dispute_arbitration / fee_collector) — coverage scaling waits on the M2 audit-prep window.

**Coverage gaps (known, not hidden).** No localnet integration test for the stake → begin_unstake → withdraw lifecycle. No bankrun timelock adapter for the lockup + cooldown + migration windows. No reward-claim test (no handler enforces it yet — A.8.8). No multi-lock test (single-stake-per-owner is structurally enforced by the seed pattern — A.8.6 invariant 7). No slashing test (no surface — A.8.8). The §4.1 conditional-PASS rider applies — fuzz coverage is on the deserialisation layer only; the instruction layer (wrong-owner / missing-signer fuzz under SVM) is queued behind the `mollusk-svm` adoption that gates the same gap on every program in the appendix.

#### A.8.10 Finding-ledger filter

**Zero `F-2026-NN` findings touched nxs_staking.** The §5.1 ledger does not currently carry an nxs_staking row; the program was scaffolded inside the M2 work band, after the M1 finding stream that drove the F-2026-01..16 entries. Same posture as A.6.10 / A.7.10 (zero findings, scaffold-stage M2). The §1.2 sweep summarised in A.8.7 is internal-review-only; the M2 audit-prep window will bring its own finding stream.

**Conditional-PASS rider** for nxs_staking is **dual + 1**: (a) the §A.8.8 *missing* surface (slashing + reward-claim + apy-authority CPI + snapshot-honesty) means the program does not yet exercise its primary roles on-chain; (b) the program has not been read against §1.2 by an external reviewer; (c) the deployed-but-inert mainnet status (distinct from A.6 / A.7's not-yet-deployed status) means a future `init_pool` is the only autonomous gate between the current scaffold and live stake — the program account is deployable-against today, with no on-chain authority change required to invoke `init_pool`. (a) + (b) close at the M2 audit-prep window; (c) closes at mainnet phase 3 of the activation sprint. Cross-cite to §6.6 for the inert-surface map (the missing-slashing + event-only `migrate_apy_authority` + missing-reward-claim belong alongside `finalize_batch` / `pay_task` / `record_job_outcome` / `execute_handler` / dispute_arbitration's 5 M2-structural CPI sites in the audit-package's stub-family enumeration, even though nxs_staking post-dates the M1 audit-package and its stub family is shaped as missing-handlers rather than as partial-implementation).
