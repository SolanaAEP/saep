...
The conditional-PASS rider relevant to proof_verifier — that the `sol_alt_bn128_group_op` syscall is unavailable in bankrun and that no localnet integration coverage exercises the verify_task → verify_proof CPI edge with a real Groth16 proof — is the public §6 (last bullet) / §7 (recently-landed services row) caveat above.
