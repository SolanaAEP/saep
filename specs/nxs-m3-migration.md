...
**Status:** spec draft. Executes at M3 alongside `specs/token2022-saep-mint.md` activation. Not actionable until FeeCollector + NXSStaking + GovernanceProgram are deployed to devnet (per token2022 §Done-checklist rows 3-5) and the rehearsal SAEP mint is initialized (row 6).
...
Migration is a one-time event per deployment (devnet rehearsal + mainnet activation). Stakers opt in per-stake: the protocol does not force-migrate locked stake. Principal preservation is guaranteed; lockup + voting-power state is reset (see §Stake-state-preservation).
...
Timeline relative to M3 mint activation T+0:

- **T+0 (activation):** Real SAEP mint init + metadata + handover complete. `apy_authority` PDA is set as the mint's `InterestBearingRateAuthority` during handover. No NXSStaking state touched yet.
...
### `migrate_apy_authority(old_mint: Pubkey, new_mint: Pubkey)` — activation-only
...
2. **Appfact off-chain swap failure path.** If Appfact misses a `Withdrawn` event and fails to mint real-SAEP, what's the staker recourse? On-chain we'd need a `claim_unswapped` ix keyed on event-id that re-triggers. Default fallback: publish a deterministic reconciliation flow keyed on event IDs.
...
7. **Token-2022 mint authority at rehearsal vs mainnet.** Rehearsal mint uses a temporary activation authority for 5 of the 6 extensions during devnet bring-up. `apy_authority` is an exception — it's the PDA from the start. Reviewer may want devnet to match mainnet handover exactly so the migration path is exercised identically. Default: devnet sets the PDA directly; mainnet uses the post-handover target. Trade-off: exercises less of the handover path on devnet.
8. **IACP announcement of migration window.** Migration open/close should emit through IACP discovery feed so portal + wallets can surface "your stake needs action." No channel exists yet; scope is a new `stake.migration.*` topic in IACP bus spec. Default: defer to the IACP cycle and require a public migration notice until the topic lands.
...
- [ ] Residual staker policy ratified (Open-Q #1) + documented in the public migration policy
- [ ] IACP topic `stake.migration.*` added to `specs/08-iacp-bus.md` OR deferred with a public migration notice fallback
- [ ] Mainnet migration checklist and go/no-go criteria published in a public release artifact
