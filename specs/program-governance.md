...
`ConfigInitialized`, `ProgramRegistered`, `ProposalCreated`, `ProposalCancelled`, `VoteCast`, `ProposalFinalized`, `ProposalQueued`, `ProposalExecuted`, `ProposalExpired`, `ProposalVetoed`, `EmergencyInvoked`, `EmergencyResolved`, `ParamsUpdated`, `AuthorityTransferProposed`, `AuthorityTransferred`.

Each event carries `proposal_id` (proposal-scoped) or `target_program` (program-scoped) plus `timestamp`. Same indexer replay model as DisputeArbitration.
