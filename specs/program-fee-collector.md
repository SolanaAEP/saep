...
- **Jito bundle assumption:** `collect_fees` + `process_epoch` are individually atomic; no multi-tx bundle dependency. The M3 mint activation flow may bundle `init_config` + mint extension writes + authority handover (see `specs/token2022-saep-mint.md`) — this program's init_config runs atomically from that flow's perspective.
